/**
 * scripts/lib/orchestrator/tools/launch-vibration.ts
 *
 * TypeScript wrapper for `launch_vibration.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'launch_vibration.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const launchVibrationTool: Tool<any, any> = {
  id: 'launch-vibration:miles-eqn',
  name: 'Launch Vibration Miles Equation',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'mechanical',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['satellite_smallsat', 'satellite_cubesat', 'satellite_geo_comsat', 'satellite_interplanetary', 'propulsion_thruster_product'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:launch-vibration:miles-eqn',
...
registerTool(launchVibrationTool)
