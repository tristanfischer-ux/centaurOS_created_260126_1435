/**
 * scripts/lib/orchestrator/tools/beamforming-codebook.ts
 *
 * TypeScript wrapper for `beamforming_codebook.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'beamforming_codebook.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const beamformingCodebookTool: Tool<any, any> = {
  id: 'beamforming-codebook:dft-hybrid',
  name: 'Beamforming Codebook DFT/Hybrid',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/rf',
  domain: 'power_electronics',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['phased_array', 'ground_station', 'satellite_smallsat'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:beamforming-codebook:dft-hybrid',
...
registerTool(beamformingCodebookTool)
