/**
 * scripts/lib/orchestrator/tools/regulatory-faa-airspace.ts
 *
 * TypeScript wrapper for `regulatory_faa_airspace.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'regulatory_faa_airspace.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const regulatoryFaaAirspaceTool: Tool<any, any> = {
  id: 'regulatory-faa:airspace',
  name: 'Regulatory FAA Airspace',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:regulatory-faa:airspace',
...
registerTool(regulatoryFaaAirspaceTool)
