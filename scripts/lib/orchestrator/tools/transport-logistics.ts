/**
 * scripts/lib/orchestrator/tools/transport-logistics.ts
 *
 * TypeScript wrapper for `transport_logistics.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/logistics
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'transport_logistics.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const transportLogisticsTool: Tool<any, any> = {
  id: 'transport-logistics:routing',
  name: 'Transport + Logistics Cost',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/logistics',
  domain: 'mechanical',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:transport-logistics:routing',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/logistics',
...
registerTool(transportLogisticsTool)
