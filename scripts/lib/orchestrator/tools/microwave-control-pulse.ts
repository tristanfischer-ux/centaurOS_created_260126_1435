/**
 * scripts/lib/orchestrator/tools/microwave-control-pulse.ts
 *
 * TypeScript wrapper for `microwave_control_pulse.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'microwave_control_pulse.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const microwaveControlPulseTool: Tool<any, any> = {
  id: 'microwave-control-pulse:drag',
  name: 'Microwave Control Pulse DRAG',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:microwave-control-pulse:drag',
...
registerTool(microwaveControlPulseTool)
