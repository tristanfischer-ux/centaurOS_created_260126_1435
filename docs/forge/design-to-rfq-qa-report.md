...
- latest local execution snapshot: `2026-02-15 18:35:48 UTC` on commit `7b85c74a`
...
- Release PR #36 is now ready for review with merge state `UNSTABLE`; merge conflicts are resolved, but branch protections/checks still need to clear before workflow-dispatch migration operations can run from `main`.
