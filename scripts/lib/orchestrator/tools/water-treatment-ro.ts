/**
 * scripts/lib/orchestrator/tools/water-treatment-ro.ts
 *
 * TypeScript wrapper for `water_treatment_ro.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'water_treatment_ro.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const waterTreatmentRoTool: Tool<any, any> = {
  id: 'water-treatment-ro:sizing',
  name: 'Water Treatment RO Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/medical',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['dialysis_machine', 'vertical_farm'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:water-treatment-ro:sizing',
...
registerTool(waterTreatmentRoTool)
