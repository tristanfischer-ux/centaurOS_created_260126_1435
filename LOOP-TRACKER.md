...
> | E | Risk: missing 1 of 3 required 40 m³ tanks (mislabeled "Cleaning Tank") | #79 #84 | **critic FALSE-POSITIVE** | INVESTIGATED (v36 state): the 3 tanks ARE present (Fresh 40m³ + Drain ×2 40m³); NO "Cleaning Tank" exists anywhere — critic's claim unsupported. REAL (subtler) defect: Fresh Water Tank + drain tanks appear in BOTH mass_fluid_transport_process AND maintenance_serviceability in moduleDecomposition (likely what made the critic miscount) — BUT requirementsBom DEDUPES to the correct count. So the dup is in design-intent, not the deliverable. NEEDS careful investigation (could be a benign maintenance cross-ref) before a gate — NOT rushing it (the G lesson) | ◑ critic-FP; dup finding logged, gate deferred |
> | F | Risk: `fertigation_dosing_system` empty words[] array | #79 | **genuine** | components scattered into `mass_fluid_transport_process`; NO enforcing gate caught it (only the flaky LLM critic) | ✅ **GATE cc7fb7946** (deterministic hollow-module check fires every time; catches F on v36). DESIGN fix (route pumps into module) = synthesis follow-up |
...
> | H | Blender render ≠ GA; dozens of floating minor components | #100 #67 #69 | **YES (real)** | DIAGNOSED by SIGHT + manifest coords (2026-06-29): NOT "two engines" — water_treatment render-blender-scene.py exit-5'd → BOTH render + GA come from build_universal_scene. The GA reads parts-manifest pos_mm DIRECTLY and is FAITHFUL (verified: TK-113 Fresh x4.3/y17.9=top-left, TK-114 x12.8/y22.9=top-centre, TK-101/102 bottom — matches general-arrangement.png). The 01-top RENDER shows tanks top-RIGHT+bottom-centre = does NOT match those coords → the RENDER is the inconsistent one, not the GA. FIX = bring the 3D render camera/scene onto the manifest coords (camera axis/perspective or envelope offset in build_universal_scene); regenerate render offline to verify (like the panel). NOT "reconcile two layouts" — make the render match the authoritative GA/manifest | ◑ diagnosed (render-side); fix = focused render increment |
...
>
> **▶ "DO ALL OF THEM" BATCH (2026-06-29, post-v37) — 3 more source fixes (v38 verifying):**
> - **TK-108 duplicate (dup family)** ✅ `0b9f81611`: reconcile re-synthesised a `nutrient_tank_synth_word` twin of the original `nutrient_tank_word` (two paths) → dup tag. Fix = run dedupePrincipalWords (name-collapse) at the END of reconcile (last synth pass). Verified on v37 state (2→1, qty-N untouched); guard principal-dedup-selftest.
> - **Determinism #86 (keystone)** ◑ `b2a958fbf`: the generator/reviewer/brief_rewriter/brief_reconciliation ran `seed:null` → non-reproducible (v36≠v37 tank set). Set `seed:42` on all design-driving stages (generator_diversity left unseeded — in-run ensemble). HONEST: provider-dependent (helps where seed honoured), reduces not guarantees variance; pairs w/ deterministic-downstream (dedup+gates). Full bit-determinism needs LLM-response caching. Verify w/ paired runs.
> - **gate-21 FATAL (part-mis-pin)** ✅ `ba0823280`: a Honeywell HSCDLNN100MDSA5 (£39 PCB board-mount SENSOR) pinned on a 'Differential-Pressure Switch' rendered £420 → 10.7× per-line FATAL. isElectronicsIcMispin keys on manufacturer (Honeywell legit) → missed it. New isBoardMountSensorMispin keys on the board-sensor MPN family (HSC/SSC/ABP/…) + field-instrument slot → rejects pin, keeps generic → gate-21 has nothing to over-price. Unit-verified (exact case); guard in emitter-mispin-selftest. Clears the FATAL next run.
>
...
>
> **▶ H ATTEMPT (2026-06-29) — diagnosed deeper, found it's BLOCKED by determinism:**
> - First hypothesis (incomer box at bbox.y1+700 floating off-deck) was WRONG — verified by regenerating the render: the change didn't move the beam. REVERTED (don't ship an unverified misdiagnosis).
> - REAL beam (route-manifest): the longest red/electrical route is **Motor Control Center → 3 Phase Power Input, span 10.1 m, detour ratio 2.97×** (route-audit max_detour_route = u_wire_trunk_…_power). It's a ROUTER DETOUR between the MCC and its supply, not the incomer placement.
> - **BLOCKER (the real finding): the LAYOUT IS NON-DETERMINISTIC.** A standalone re-render of the SAME v38 state produced a COMPLETELY DIFFERENT layout than v38's stored render — nutrient tanks moved x≈19/y≈5 → x≈9/y≈22. So the stray beam + the part spread SHIFT run-to-run; a render fix can't be reliably verified offline (standalone render ≠ chain render), and I'd be chasing a moving target.
> - CONCLUSION: **H is gated by determinism (#86), not the other way round.** The correct order is: make place_all deterministic (same state → same positions) FIRST, THEN fix the MCC↔3-Phase co-location / router detour with reliable verification. Trying H directly hit the determinism wall concretely. (Also relevant: the render's per-run layout variance is itself part of why the render tabs score is a treadmill.)
>
> **▶ LAYOUT DETERMINISM (#86) — root-caused 2026-06-29, fix = the deterministic rewrite Tristan specified:**
> - CONFIRMED non-deterministic: same v38 state rendered twice → 52 DIFFERENT part positions each run (tag-independent position SETS differ). It's not tagging — the geometry genuinely moves.
> - **ROOT MECHANISM: Blender's bundled Python FORCES hash randomisation ON — it IGNORES PYTHONHASHSEED** (verified: hash_randomization=1 + hash('forge') varies even with PYTHONHASHSEED=0). So every string-`set`/`dict` iteration in the placement path orders differently per process. Can't be fixed by an env var.
> - NARROWED: flow-region ordering + banking + periphery ARE deterministic (identical across runs, from the `[univ][flow]` logs). `extract_parts`, `_shelf_pack`, `_place_grid`, `_place_region` all iterate ordered lists. Pinning the `parts` input order at place_all top did NOT fix it (reverted). So the residual source is inside `build_part` / `forge_blender_lib` / a Blender-object-iteration step — not pinned by reading; would need an instrumented position-trace build.
> - **DECISION (Tristan's spec, 2026-06-29):** do NOT keep patching the organically-grown placer. REWRITE the layout as a DETERMINISTIC SEQUENCE PACKER: components in process order (region_rank), placed as close together as possible; small standard items (instruments, small devices) CONSOLIDATED INTO CABINETS (10-30 small parts → one cabinet box) instead of scattered across the floor; driven by better component SIZING. A deterministic algorithm has zero run-to-run variance → inherently fixes #86 AND delivers the clean co-located layout + kills the render treadmill + unblocks H. (Cabinet consolidation partly exists: `_consolidate_control_cabinet` — extend it universally.)
>
> **▶ DETERMINISM #86 — DEEP DIVE 2026-06-29 (Tristan: "the design must be deterministic, not generative; Blender is the source of truth the GA + drawings read from"). PARTIAL — root-caused to MULTIPLE layers:**
> - MECHANISM: Blender's bundled Python FORCES hash randomisation ON + IGNORES PYTHONHASHSEED (verified hash_randomization=1, hash('forge') varies even with seed=0). Every set/dict-of-strings iteration in placement orders differently per process.
> - **FIXED + committed (82a5a6af5, d3399290d):** (1) connection-ledger endpoint closers — 8 sites resolved dangling edges with min()/sorted() keyed on RANK ONLY; ties broke by set order → DIFFERENT edge set per run. Now total-order (rank,name). **EDGES verified deterministic.** (2) layout_optimiser: canonicalise edges before float-accumulating incident (float add not associative) + round order key; (3) _resolve nameset sorted; (4) common.sort total-order; (5) optimiser equip nodes sorted by (name,_seq); Part._seq = det extraction index. (6) **determinism_check.py BUILT** — renders a state twice, asserts bit-identical positions+edges (the permanent guard; run `python3 scripts/blender-universal/determinism_check.py <state.json>`).
...
> Status: **DETERMINISM ACHIEVED (flag-gated) — the #1 goal is met.** B1 (deterministic_layout.py core: pure, order-invariant, integer-grid, cabinet-consolidating; selftest in verify-engine-guards) = 73585fb74. B2 (wired behind DETERMINISTIC_PLACEMENT=1: _populate_det_layout → dl.layout() → _DET_LAYOUT_POS → _shelf_pack uses the absolute coord) = 41374f561. VERIFIED bit-identical across renders: water 5/5, BESS 2/2, CO2 ✓ — all uncrashed; water render co-located + coherent. Guard + edge-determinism = 82a5a6af5, d3399290d.
> **✅ DEFAULT-ON + VERIFIED (524cd9b22, Tristan signed off "flip the default"):** square width-budget banking + skip optimiser/compaction when det (cad451d30). Default render (NO flag) bit-identical 2/2, square + sequenced + centred + fills envelope. **GA now READS THE SAME det manifest as the render → they MATCH** (item H structurally resolved: tanks cluster bottom-left in BOTH; GA envelope 30.7×39.8 m). **Full engine guard sweep PASSES (exit 0, zero FAILs)** — no regression. ENV repaired this session: system Node reset to v25 (esbuild/eslint/tsx crash exit 194) → use /opt/homebrew/opt/node@22 (.node-version=22); .venv + node_modules had circular-symlink corruption → rebuilt (`python3 -m venv .venv` + openpyxl/numpy/matplotlib/cairosvg/httpx; `npm ci`).
> **REMAINING B polish (non-blocking):** (a) surface dl's cabinet node as ONE drawn box (logic exists, draw step still renders members individually); (b) utility-incomer stray beam (pre-existing in BOTH engines — route incomer beside its switchboard); (c) CO2 slightly left-shifted; (d) full chain on Codema → re-attack dossier with stable geometry underneath.
>
