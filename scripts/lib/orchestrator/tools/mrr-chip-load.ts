/**
 * scripts/lib/orchestrator/tools/mrr-chip-load.ts
 *
 * TypeScript wrapper for `mrr_chip_load.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/cnc
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'mrr_chip_load.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const mrrChipLoadTool: Tool<any, any> = {
  id: 'mrr-chip-load:milling',
  name: 'Material Removal Rate + Chip Load',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/cnc',
  domain: 'mechanical',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['cnc_machine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:mrr-chip-load:milling',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/cnc',
