/**
 * scripts/lib/orchestrator/tools/heat-pipe-sizing.ts
 *
 * TypeScript wrapper for `heat_pipe_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'heat_pipe_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const heatPipeSizingTool: Tool<any, any> = {
  id: 'heat-pipe:sizing',
  name: 'Heat Pipe Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/thermal',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['satellite_smallsat', 'satellite_geo_comsat', 'satellite_interplanetary', 'edge_ai', 'cnc_machine'].includes(envelope.class) },
...
registerTool(heatPipeSizingTool)
