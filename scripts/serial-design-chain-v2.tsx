...
(C0) UNIT SCALE — MANDATORY: state every quantity in the unit that gives it 3+ significant figures at THIS design's magnitude. A 14.1 kWh pack is "14.1 kWh", NEVER "0.01 MWh" (a 1-sig-fig unit conversion destroys the number and reads as a 100× error); a 3,500 kWh plant is "3.5 MWh". Apply to energy (kWh vs MWh), power (kW vs MW), mass (kg vs t) alike.
...
async function main() {
  ...
  // NB hollow-module reconcile is NOT done here — the emptying happens LATE (a downstream word-mutating
  // stage empties a sub-module after these normalisers), so it runs as a Python pass on the settled
  // state.json after dossier_repair (see reconcile_hollow.py, ~line 8740).
  ...
  // ── HOLLOW-MODULE RECONCILE on the FINAL state.json (2026-07-01). A sub-module that DESCRIBES
  //    components (prose ≥5 words) but carries ZERO words is a hollow shell — its parts were scattered
  //    into another module, so it reads as described-but-empty (the deterministic hollow-module check +
  //    the physics critic both flag it, dragging Risk & ⚠Checks <8). This MUST run on the settled
  //    state.json (not in-memory post-Phase-2) because the emptying happens LATE — a downstream
  //    word-mutating stage (population consolidation / reconcile / dossier_repair) empties the sub-module
  //    AFTER the earlier normalisers, so an in-memory pass sees moved=0 (the "synthesis runs in two paths"
  //    trap). Un-scatters the best token-matching word back in, else demotes orphan prose. Idempotent.
  try {
    execFileSync('python3', [resolve(__dirname, 'lib', 'reconcile_hollow.py'), outDir],
      { stdio: 'inherit', timeout: 60_000 })
    logAction({ step: 'reconcile_hollow', ok: true })
  } catch (herr) {
    console.error(`[chain] hollow-module reconcile failed (non-fatal): ${(herr as Error).message.slice(0, 140)}`)
  ...
  // EXCEL-ONLY DELIVERABLE (Tristan 2026-06-23, emphatic): the Excel spreadsheet IS the product;
  // the react-pdf chain-v2.pdf is NOT wanted, EVER, by default. PDF rendering is now OFF unless
  // CHAIN_WANT_PDF=1 is explicitly set. It is also skipped when Blender renders are skipped
  // (CHAIN_SKIP_RENDER=1) or before the quality loop converges. The Excel deliverable is built
  // above (excel_deliverable step) regardless, so a normal run yields the full Excel and NO PDF.
  if (process.env.CHAIN_WANT_PDF !== '1' || process.env.CHAIN_SKIP_RENDER === '1' || QUALITY_LOOP_PHASE < 3) {
    const _why = process.env.CHAIN_WANT_PDF !== '1' ? 'PDF OFF by default (set CHAIN_WANT_PDF=1 to enable)'
      : process.env.CHAIN_SKIP_RENDER === '1' ? 'CHAIN_SKIP_RENDER=1'
      : `QUALITY_LOOP_PHASE=${QUALITY_LOOP_PHASE}`
    console.error(`[chain] ${_why} — NOT rendering chain-v2.pdf (Excel deliverable + drawings + BoM are on disk; quality loop still runs)`)
