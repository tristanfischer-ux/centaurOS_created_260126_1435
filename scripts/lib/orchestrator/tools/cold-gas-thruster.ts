/**
 * scripts/lib/orchestrator/tools/cold-gas-thruster.ts
 *
 * TypeScript wrapper for `cold_gas_thruster.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'cold_gas_thruster.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const coldGasThrusterTool: Tool<any, any> = {
  id: 'cold-gas-thruster:sizing',
  name: 'Cold Gas Thruster Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['propulsion_thruster_product', 'satellite_cubesat', 'satellite_smallsat'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:cold-gas-thruster:sizing',
...
registerTool(coldGasThrusterTool)
