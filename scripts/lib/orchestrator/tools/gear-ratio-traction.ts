/**
 * scripts/lib/orchestrator/tools/gear-ratio-traction.ts
 *
 * TypeScript wrapper for `gear_ratio_traction.py` — subprocess to repo .venv.
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'gear_ratio_traction.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const GearRatioTractionTool: Tool<any, any> = {
  id: 'gear:traction-ratio',
  name: 'Traction Gear Ratio Wheel↔MGU Map',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    const provenance: Provenance = {
      source: 'tool:gear:traction-ratio',
...
registerTool(GearRatioTractionTool)
