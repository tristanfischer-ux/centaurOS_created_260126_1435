/**
 * scripts/lib/orchestrator/tools/corrosion-anode-sizing.ts
 *
 * TypeScript wrapper for `corrosion_anode_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'corrosion_anode_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const corrosionAnodeSizingTool: Tool<any, any> = {
  id: 'corrosion:anode-sizing',
  name: 'Sacrificial Anode Sizing',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:corrosion:anode-sizing',
...
registerTool(corrosionAnodeSizingTool)
