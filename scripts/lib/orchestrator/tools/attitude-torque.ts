/**
 * scripts/lib/orchestrator/tools/attitude-torque.ts
 *
 * TypeScript wrapper for `attitude_torque.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'attitude_torque.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const attitudeTorqueTool: Tool<any, any> = {
  id: 'attitude:disturbance-torque',
  name: 'Attitude Disturbance Torque',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:attitude:disturbance-torque',
...
registerTool(attitudeTorqueTool)
