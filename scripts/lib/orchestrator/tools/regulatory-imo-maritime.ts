/**
 * scripts/lib/orchestrator/tools/regulatory-imo-maritime.ts
 *
 * TypeScript wrapper for `regulatory_imo_maritime.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'regulatory_imo_maritime.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const regulatoryImoMaritimeTool: Tool<any, any> = {
  id: 'regulatory-imo:maritime',
  name: 'Regulatory IMO Maritime',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:regulatory-imo:maritime',
...
registerTool(regulatoryImoMaritimeTool)
