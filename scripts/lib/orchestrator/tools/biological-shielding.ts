/**
 * scripts/lib/orchestrator/tools/biological-shielding.ts
 *
 * TypeScript wrapper for `biological_shielding.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'biological_shielding.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const biologicalShieldingTool: Tool<any, any> = {
  id: 'smr:biological-shielding',
  name: 'SMR Biological Shielding',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/nuclear',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['smr'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:smr:biological-shielding',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/nuclear',
...
registerTool(biologicalShieldingTool)
