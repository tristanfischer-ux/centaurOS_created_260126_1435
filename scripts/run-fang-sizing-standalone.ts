...
async function main() {
    console.log("Starting standalone Fang Sizing pipeline...");
    ...
    await supabase.from('cad_lab_pipeline_runs')
        ...
        .eq('stage', 'fang.sizing');
    
    console.log("Calling runFangSizingBackground...");
    try {
        const result = await runFangSizingBackground(projectId, foundryId, userId, "manual");
        console.log("=== RESULT ===");
