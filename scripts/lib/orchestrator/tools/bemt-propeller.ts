/**
 * scripts/lib/orchestrator/tools/bemt-propeller.ts
 *
 * TypeScript wrapper for `bemt_propeller.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'bemt_propeller.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const bemtPropellerTool: Tool<any, any> = {
  id: 'bemt-propeller:thrust',
  name: 'BEMT Propeller Thrust',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/aero',
  domain: 'aero',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['drone', 'haps', 'auv', 'wind_turbine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:bemt-propeller:thrust',
...
registerTool(bemtPropellerTool)
