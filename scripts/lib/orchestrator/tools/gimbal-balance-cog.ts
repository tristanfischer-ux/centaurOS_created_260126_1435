/**
 * scripts/lib/orchestrator/tools/gimbal-balance-cog.ts
 *
 * TypeScript wrapper for `gimbal_balance_cog.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'gimbal_balance_cog.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const gimbalBalanceCogTool: Tool<any, any> = {
  id: 'gimbal:cog-balance',
  name: 'Gimbal Centre-of-Gravity Balance',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:gimbal:cog-balance',
...
registerTool(gimbalBalanceCogTool)
