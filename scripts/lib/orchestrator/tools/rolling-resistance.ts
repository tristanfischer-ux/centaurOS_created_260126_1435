/**
 * scripts/lib/orchestrator/tools/rolling-resistance.ts
 *
 * TypeScript wrapper for `rolling_resistance.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/ebike
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'rolling_resistance.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const rollingResistanceTool: Tool<any, any> = {
  id: 'rolling-resistance:cycling',
  name: 'Rolling Resistance Cycling Power',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/ebike',
  domain: 'mechanical',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['e_bike'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:rolling-resistance:cycling',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/ebike',
...
registerTool(rollingResistanceTool)
