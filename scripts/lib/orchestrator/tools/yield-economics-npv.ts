/**
 * scripts/lib/orchestrator/tools/yield-economics-npv.ts
 *
 * TypeScript wrapper for `yield_economics_npv.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/finance
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'yield_economics_npv.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const yieldEconomicsNpvTool: Tool<any, any> = {
  id: 'yield-economics:npv',
  name: 'Yield + Economics NPV',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:yield-economics:npv',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/finance',
...
registerTool(yieldEconomicsNpvTool)
