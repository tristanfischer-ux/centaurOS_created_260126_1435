/**
 * scripts/lib/orchestrator/tools/electric-propulsion-sizing.ts
 *
 * TypeScript wrapper for `electric_propulsion_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'electric_propulsion_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const electricPropulsionSizingTool: Tool<any, any> = {
  id: 'electric-propulsion:sizing',
  name: 'Electric Propulsion Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'power_electronics',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['propulsion_thruster_product', 'satellite_geo_comsat', 'satellite_interplanetary'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:electric-propulsion:sizing',
...
registerTool(electricPropulsionSizingTool)
