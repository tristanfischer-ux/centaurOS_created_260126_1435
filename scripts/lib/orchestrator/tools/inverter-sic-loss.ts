/**
 * scripts/lib/orchestrator/tools/inverter-sic-loss.ts
 *
 * TypeScript wrapper for `inverter_sic_loss.py` — subprocess to repo .venv.
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'inverter_sic_loss.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const InverterSicLossTool: Tool<any, any> = {
  id: 'inverter:sic-loss',
  name: 'SiC Inverter Conduction + Switching Loss',
  version: '1.0.0',
...
registerTool(InverterSicLossTool)
