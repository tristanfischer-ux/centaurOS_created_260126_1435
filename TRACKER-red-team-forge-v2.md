...
| B6 | RT.2 | P2 | `investor-deck-button.tsx` | `atob` / blob construction was unguarded. A malformed server response would throw `InvalidCharacterError` inside the useTransition callback — unhandled client crash rather than a user-facing error. | ✅ Wrapped decode + download in try/catch; catch logs to console + shows a user-facing toast; URL.revokeObjectURL runs in finally. |
