/**
 * scripts/lib/orchestrator/tools/membrane-humidification.ts
 *
 * TypeScript wrapper for `membrane_humidification.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'membrane_humidification.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const membraneHumidificationTool: Tool<any, any> = {
  id: 'pemfc:membrane-humidification',
  name: 'PEMFC Membrane Humidification',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:pemfc:membrane-humidification',
...
registerTool(membraneHumidificationTool)
