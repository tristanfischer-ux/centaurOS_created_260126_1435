...
-- Migration: 20260212300007_seed_tier2_expansion
-- ============================================================
-- Seeds 13 additional electromechanical geometry types:
--   Actuators (3), Switching (1), Power (2), Display (1),
--   Switches (1), Thermal (1), Connectors (1), Indicators (2), Input (1), Power (1)
...
$CADQUERY$,
    '{"body_l": {"type": "number", "default": 40.0, "unit": "mm"}, "body_w": {"type": "number", "default": 20.0, "unit": "mm"}, "horn_length": {"type": "number", "default": 18.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'actuator'],
    '#2F4F4F',
    'Hobby RC servo motor with output horn and mounting ears.',
    TRUE
...
$CADQUERY$,
    '{"motor_d": {"type": "number", "default": 12.0, "unit": "mm"}, "shaft_d": {"type": "number", "default": 3.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['metal', 'motor'],
    '#C0C0C0',
    'Small DC geared motor (N20 / GA12 style).',
...
$CADQUERY$,
    '{"body_d": {"type": "number", "default": 15.0, "unit": "mm"}, "plunger_d": {"type": "number", "default": 5.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['metal', 'electromagnetic'],
    '#2F2F2F',
    'Linear push/pull solenoid.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def relay_module(params):
    ...
    return body
$CADQUERY$,
    '{"body_l": {"type": "number", "default": 19.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'electromagnetic'],
    '#000080',
    'Electromechanical relay (sugar cube style).',
    TRUE
) ON CONFLICT (slug) DO NOTHING;

-- 5. voltage_regulator_module
INSERT INTO component_geometry_types (slug, name, tier, category, cadquery_code, param_schema, mounting_interfaces, visual_tags, default_colour, description, verified)
VALUES (
...
$CADQUERY$,
    '{"pcb_l": {"type": "number", "default": 43.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['pcb', 'power'],
    '#006400',
    'DC-DC buck/boost converter module (LM2596 style).',
...
$CADQUERY$,
    '{"pcb_w": {"type": "number", "default": 27.0, "unit": "mm"}, "display_w": {"type": "number", "default": 22.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['pcb', 'display'],
    '#1A1A1A',
    'Small OLED/LCD display module (0.96" or 1.3").',
...
$CADQUERY$,
    '{"body_l": {"type": "number", "default": 20.0, "unit": "mm"}, "lever_length": {"type": "number", "default": 25.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'switch'],
    '#1A1A1A',
    'Microswitch / limit switch with lever actuator.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def heatsink_extruded(params):
    ...
    return base
$CADQUERY$,
    '{"length": {"type": "number", "default": 40.0, "unit": "mm"}, "width": {"type": "number", "default": 40.0, "unit": "mm"}, "fin_count": {"type": "integer", "default": 10}}'::jsonb,
    '[]'::jsonb,
    ARRAY['aluminium', 'thermal'],
    '#1A1A1A',
    'Extruded aluminium fin heatsink.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def terminal_block(params):
    ...
    return body
$CADQUERY$,
    '{"positions": {"type": "integer", "default": 4}, "pitch": {"type": "number", "default": 5.08, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'connector'],
    '#006400',
    'Screw terminal block / barrier strip.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
$CADQUERY$,
    '{"led_d": {"type": "number", "default": 5.0, "unit": "mm", "enum": [3, 5, 8, 10]}}'::jsonb,
    '[]'::jsonb,
    ARRAY['led', 'indicator'],
    '#FF0000',
    'Panel-mount LED indicator (3mm or 5mm).',
...
def piezo_buzzer(params):
    ...
    return body
$CADQUERY$,
    '{"diameter": {"type": "number", "default": 12.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'audio'],
    '#1A1A1A',
    'Piezo buzzer / sounder.',
...
$CADQUERY$,
    '{"body_d": {"type": "number", "default": 12.0, "unit": "mm"}, "shaft_d": {"type": "number", "default": 6.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['metal', 'input'],
    '#808080',
    'Rotary encoder with push-button and knob.',
...
def battery_holder_18650(params):
    ...
    return body
$CADQUERY$,
    '{"cells": {"type": "integer", "default": 1, "enum": [1, 2, 3, 4]}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'power'],
    '#1A1A1A',
    '18650 battery holder clip (single or dual).',
