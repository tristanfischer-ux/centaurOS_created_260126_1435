/**
 * scripts/lib/orchestrator/tools/rotor-tilt-transition.ts
 *
 * TypeScript wrapper for `rotor_tilt_transition.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'rotor_tilt_transition.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const rotorTiltTransitionTool: Tool<any, any> = {
  id: 'rotor-tilt-transition:dynamics',
  name: 'Tilt-Rotor Transition Dynamics',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/aero',
  domain: 'aero',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['evtol'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:rotor-tilt-transition:dynamics',
...
registerTool(rotorTiltTransitionTool)
