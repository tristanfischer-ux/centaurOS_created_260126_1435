/**
 * scripts/lib/orchestrator/tools/gear-ratio.ts
 *
 * TypeScript wrapper for `gear_ratio.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'gear_ratio.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const gearRatioTool: Tool<any, any> = {
  id: 'gear-ratio:bicycle',
  name: 'Bicycle Gear Ratio + Speed',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:gear-ratio:bicycle',
