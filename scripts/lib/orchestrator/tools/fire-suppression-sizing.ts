/**
 * scripts/lib/orchestrator/tools/fire-suppression-sizing.ts
 *
 * TypeScript wrapper for `fire_suppression_sizing.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/safety
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'fire_suppression_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const fireSuppressionSizingTool: Tool<any, any> = {
  id: 'fire-suppression:nfpa',
  name: 'Fire Suppression Sizing NFPA',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/safety',
  domain: 'standards',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['bess', 'h2_electrolyser'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:fire-suppression:nfpa',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/safety',
...
registerTool(fireSuppressionSizingTool)
