/**
 * scripts/lib/orchestrator/tools/glucose-sensor.ts
 *
 * TypeScript wrapper for `glucose_sensor.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'glucose_sensor.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const glucoseSensorTool: Tool<any, any> = {
  id: 'glucose-sensor:amperometric',
  name: 'Glucose Amperometric Biosensor',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:glucose-sensor:amperometric',
...
registerTool(glucoseSensorTool)
