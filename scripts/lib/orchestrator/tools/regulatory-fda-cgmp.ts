/**
 * scripts/lib/orchestrator/tools/regulatory-fda-cgmp.ts
 *
 * TypeScript wrapper for `regulatory_fda_cgmp.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'regulatory_fda_cgmp.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const regulatoryFdaCgmpTool: Tool<any, any> = {
  id: 'regulatory-fda:cgmp',
  name: 'Regulatory FDA cGMP',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/regulatory',
  domain: 'standards',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['bioreactor', 'cgm', 'ventilator', 'dialysis_machine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:regulatory-fda:cgmp',
...
registerTool(regulatoryFdaCgmpTool)
