/**
 * scripts/lib/orchestrator/tools/acoustic-modem-link.ts
 *
 * TypeScript wrapper for `acoustic_modem_link.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'acoustic_modem_link.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const acousticModemLinkTool: Tool<any, any> = {
  id: 'acoustic-modem:link-budget',
  name: 'Acoustic Modem Link Budget',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:acoustic-modem:link-budget',
...
registerTool(acousticModemLinkTool)
