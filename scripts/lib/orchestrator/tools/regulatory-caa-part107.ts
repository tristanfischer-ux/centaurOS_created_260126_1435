/**
 * scripts/lib/orchestrator/tools/regulatory-caa-part107.ts
 *
 * TypeScript wrapper for `regulatory_caa_part107.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'regulatory_caa_part107.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const regulatoryCaaPart107Tool: Tool<any, any> = {
  id: 'regulatory-caa:part107',
  name: 'Regulatory CAA Part 107',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/regulatory',
  domain: 'standards',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['drone'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:regulatory-caa:part107',
...
registerTool(regulatoryCaaPart107Tool)
