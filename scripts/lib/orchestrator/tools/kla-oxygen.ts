/**
 * scripts/lib/orchestrator/tools/kla-oxygen.ts
 *
 * TypeScript wrapper for `kla_oxygen.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'kla_oxygen.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const klaOxygenTool: Tool<any, any> = {
  id: 'kla-oxygen:transfer',
  name: 'kLa Oxygen Transfer Coefficient',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/bio',
  domain: 'biochemistry',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['bioreactor', 'dialysis_machine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:kla-oxygen:transfer',
