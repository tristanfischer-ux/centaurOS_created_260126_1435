/**
 * scripts/lib/orchestrator/tools/gust-response-drone.ts
 *
 * TypeScript wrapper for `gust_response_drone.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'gust_response_drone.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const gustResponseDroneTool: Tool<any, any> = {
  id: 'gust-response:drone',
  name: 'Drone Gust Response',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:gust-response:drone',
...
registerTool(gustResponseDroneTool)
