...
export function CadLabProvider({ children }: { children: ReactNode }): ReactNode {
  ...
  // INTENT: pendingReviewKeys persisted to sessionStorage keyed by projectId.
  // Without this, F5 during a batch review dropped the pending flag — reviews
  // in flight either double-fired on retry or appeared stuck with no CTA.
  // sessionStorage (not localStorage) because pending state shouldn't survive
  // a browser close — the request will have completed or timed out by then.
  const pendingReviewStorageKey = activeProjectId ? `forge-pending-reviews-${activeProjectId}` : null

  const [pendingReviewKeys, setPendingReviewKeys] = useState<Set<string>>(() => {
    if (!pendingReviewStorageKey || typeof window === "undefined") return new Set()
    try {
      const stored = sessionStorage.getItem(pendingReviewStorageKey)
      if (stored) return new Set(JSON.parse(stored) as string[])
    } catch (err) {
      console.warn("[CAD-LAB] Failed to hydrate pendingReviewKeys:", err)
    }
    return new Set()
  })
  ...
  useEffect(() => {
    if (!pendingReviewStorageKey || typeof window === "undefined") return
    try {
      if (pendingReviewKeys.size === 0) {
        sessionStorage.removeItem(pendingReviewStorageKey)
      } else {
        sessionStorage.setItem(pendingReviewStorageKey, JSON.stringify([...pendingReviewKeys]))
      }
    } catch (err) {
      console.warn("[CAD-LAB] Failed to persist pendingReviewKeys:", err)
    }
  }, [pendingReviewStorageKey, pendingReviewKeys])

  // Rehydrate when the project changes (mirrors the Source-page rehydrate).
  const prevPendingProjectIdRef = useRef<string | null>(activeProjectId)
  useEffect(() => {
    if (activeProjectId === prevPendingProjectIdRef.current) return
    prevPendingProjectIdRef.current = activeProjectId
    if (!pendingReviewStorageKey || typeof window === "undefined") {
      setPendingReviewKeys(new Set())
      return
    }
    try {
      const stored = sessionStorage.getItem(pendingReviewStorageKey)
      setPendingReviewKeys(stored ? new Set(JSON.parse(stored) as string[]) : new Set())
    } catch {
      setPendingReviewKeys(new Set())
    }
  }, [activeProjectId, pendingReviewStorageKey])
  ...
  const handleReset = useCallback(() => {
    // Capture the departing project id BEFORE state is cleared — we'll use
    // it to sweep the per-project localStorage keys owned by Source + Assemble.
    const departingProjectId = activeProjectIdRef.current
    ...
    if (typeof window !== "undefined") {
      ...
      // INTENT: Source + Assemble pages cache per-project state in localStorage
      // under these prefixes. Resetting the CAD Lab context without sweeping
      // these keys leaks state across project boundaries: Project B could
      // inherit Project A's supplier matches / shortlist / buy-part results /
      // assembly tiers / branding / shipping until the localStorage value
      // expires naturally (never). Sweep exactly the keys for the departing
      // project — leave other projects' caches alone.
      if (departingProjectId) {
        const perProjectKeys = [
          `forge-supplier-matches-${departingProjectId}`,
          `forge-supplier-shortlist-v2-${departingProjectId}`,
          `forge-supplier-shortlist-v3-${departingProjectId}`,
          `forge-supplier-rfqs-${departingProjectId}`,
          `forge-buy-search-v7-${departingProjectId}`,
          `forge-assembly-matches-${departingProjectId}`,
          `forge-assembly-tiers-${departingProjectId}`,
          `forge-assembly-branding-${departingProjectId}`,
          `forge-assembly-shipping-${departingProjectId}`,
        ]
        for (const key of perProjectKeys) {
          try { localStorage.removeItem(key) } catch (e) { console.warn("[CAD-LAB] reset localStorage sweep failed for", key, e) }
        }
        // sessionStorage: pending reviews (see pendingReviewKeys persistence)
        try { sessionStorage.removeItem(`forge-pending-reviews-${departingProjectId}`) } catch (e) { console.warn("[CAD-LAB] reset sessionStorage sweep failed", e) }
      }
    }
    setSubject("")
    setResearchResult(null)
