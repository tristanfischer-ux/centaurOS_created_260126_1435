...
- riskMatrix is REQUIRED (Loop 5 P5 + Loop 6 verification): you MUST emit at least one riskMatrix row PER failureMode entry on every module. The first Loop 5 regen revealed Sonnet was silently dropping this field — the council scored unranked failureModes string lists 3-4/10 and structured FMEA matrices 8/10. Treat the riskMatrix field as the canonical risk register; failureModes string array is a legacy mirror, not a substitute. Every module's riskMatrix.length MUST equal failureModes.length (same hazards, structured representation). severity 1-5 (1=negligible, 3=major operational disruption, 5=catastrophic / safety-critical / total loss). likelihood 1-5 (1=very rare / once in 10 years, 3=plausible / yearly, 5=likely / monthly+). mitigation must be a concrete action, not "review later". owner must be a role, not "TBD". residualSeverity / residualLikelihood = rating AFTER mitigation lands. Skipping this field means the founder cannot prioritise risks for investor or insurer conversations and the report scores fail dimension 8.
...
- FUNCTIONAL-ROLE DEDUPLICATION (Loop 6 P3, Gemini-specific): for each FUNCTIONAL ROLE — primary load-bearing structure, sealing/IP rating, thermal management, EMI shielding, electrical isolation — list the modules/parts that fulfil it, AND make sure the same role isn't fulfilled redundantly by multiple modules. Example failure (Hedgerow Loop 5): the engine emitted "Outer steel shell" + "Inner steel shell" + "Aluminium die-cast chassis" — three modules all providing primary structural rigidity for a £155 garden bird feeder. ONE primary load-bearing enclosure is correct; secondary structural nodes should only appear when thermal isolation, EMI compartmentation, or IP-rated sub-enclosure constraints explicitly demand them. When in doubt, prune. The cost waterfall is unforgiving of redundant structural mass.
...
function heroImagePromptSchemaForStyle(style: _IllustrationStyle): string {
  switch (style) {
    ...
    case "photography":
      return `"heroImagePrompt": "300-500 words. Complete self-contained EDITORIAL LIFESTYLE PHOTOGRAPHY prompt. Write as a brief for a magazine-ready brand photograph. START with: 'Editorial lifestyle photograph of the fully assembled product, in-situ, shot on a full-frame mirrorless camera with a 50mm prime lens...'. Describe the product's deployment environment as the backdrop (workbench, laboratory bench, factory floor, outdoor field, desk — pick the most authentic to the product's use). Describe natural or naturalistic lighting (window light, overcast daylight, warm tungsten, diffused softbox). Photorealistic materials with convincing wear, dust, fingerprints, real reflections. Shallow depth-of-field — subject tack sharp, background gently defocused. Composition follows the rule of thirds with generous negative space for press crops. Camera: 30-45-degree hero angle, eye-level or slightly above. NO blueprint, NO exploded view, NO engineering grid, NO callout arrows, NO text/labels. End with: editorial photography, lifestyle context, natural lighting, photorealistic materials, shallow depth-of-field. Mirror-pair parts MUST still share identical material, finish, colour, and surface detail."`
  }
}
...
function perModuleImagePromptSchemaForStyle(style: _IllustrationStyle): string {
  switch (style) {
    ...
    case "photography":
      return `"module-id": "150-200 words. START with: 'Editorial close-up photograph of this module as seen on the assembled product in its deployment environment...'. Shallow depth-of-field, natural lighting, matching the hero image's palette and mood. Real-world surface texture — dust, slight wear, realistic reflections. NO blueprint lines, NO engineering grid, NO callout arrows, NO text/labels. Mirror-pair modules MUST get IDENTICAL prompt text."`
  }
}
