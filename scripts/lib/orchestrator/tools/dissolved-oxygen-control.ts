/**
 * scripts/lib/orchestrator/tools/dissolved-oxygen-control.ts
 *
 * TypeScript wrapper for `dissolved_oxygen_control.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'dissolved_oxygen_control.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const dissolvedOxygenControlTool: Tool<any, any> = {
  id: 'dissolved-oxygen:control',
  name: 'Dissolved Oxygen Control',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:dissolved-oxygen:control',
...
registerTool(dissolvedOxygenControlTool)
