/**
 * scripts/lib/orchestrator/tools/power-module-sizing.ts
 *
 * TypeScript wrapper for `power_module_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'power_module_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const powerModuleSizingTool: Tool<any, any> = {
  id: 'power-module:sizing',
  name: 'Modular DC Power Module Sizing',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:power-module:sizing',
...
registerTool(powerModuleSizingTool)
