/**
 * scripts/lib/orchestrator/tools/mli-thermal.ts
 *
 * TypeScript wrapper for `mli_thermal.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'mli_thermal.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const mliThermalTool: Tool<any, any> = {
  id: 'mli:multi-layer-insulation',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:mli:multi-layer-insulation',
