...
def main():
    ...
    A("")
    A("Tony's CAD (received 24 Jul; SketchUp file *24-hex sub array, 3.1 mm between flats, "
      "150 micron wall*) fixes the cell lattice: hexagonal cells **3.1 mm across-flats, "
      "150 µm walls, 7.7 mm deep** (depth from the dimensioned 3D CAD; the .skp is the 2D "
      "tileable profile), 3D-printed (metallised for RF duty), tiled as **24-hex "
      "sub-arrays**; demo-scale prototypes (≈25–30 mm cells, hex and square egg-crate) "
      "photographed working. Scope note: the cell-size↔band mapping is RF and stays with "
      "Tony/Vlad — cell dimensions are treated as INPUTS here; only structure, packing and "
      "mass are computed.")
    A("")
    A("| Quantity | Formula | Substitution | Result |")
    A("|---|---|---|---|")
    ...
    A("| Cells per sub-array | from the .skp | — | 24 (aperture = tiles × 24) |")
    A("| Relative density | 2t/AF (shared walls) | 2×0.15/3.1 | 0.097 |")
    A("| Wall volume per cell | √3·AF·t·L | √3×3.1×0.15×7.7 | 6.20 mm³ |")
    A("| Cell wall mass | V·ρ | 6.20 mm³ × 1.24 mg/mm³ (printed) | 7.7 mg |")
    A("| Lattice mass per sub-array | N·m | 24 × 7.7 mg | **0.185 g** (Al: 0.40 g) |")
    ...
    A("| Actuators per sub-array | N·m | 24 × 220 mg | **5.3 g** — dominates the lattice ≈29× |")
    A("")
