/**
 * scripts/lib/orchestrator/tools/pest-control-uvc.ts
 *
 * TypeScript wrapper for `pest_control_uvc.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'pest_control_uvc.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const pestControlUvcTool: Tool<any, any> = {
  id: 'pest-control:uvc-dose',
  name: 'UVC Pest Control Dose',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:pest-control:uvc-dose',
...
registerTool(pestControlUvcTool)
