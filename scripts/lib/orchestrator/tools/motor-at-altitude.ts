/**
 * scripts/lib/orchestrator/tools/motor-at-altitude.ts
 *
 * TypeScript wrapper for `motor_at_altitude.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/motors
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'motor_at_altitude.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const motorAtAltitudeTool: Tool<any, any> = {
  id: 'motor:altitude-derating',
  name: 'Motor Altitude Derating',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/motors',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:motor:altitude-derating',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/motors',
...
registerTool(motorAtAltitudeTool)
