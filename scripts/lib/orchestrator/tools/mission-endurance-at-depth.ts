/**
 * scripts/lib/orchestrator/tools/mission-endurance-at-depth.ts
 *
 * TypeScript wrapper for `mission_endurance_at_depth.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'mission_endurance_at_depth.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const missionEnduranceAtDepthTool: Tool<any, any> = {
  id: 'mission-endurance:depth',
  name: 'Mission Endurance at Depth',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:mission-endurance:depth',
...
registerTool(missionEnduranceAtDepthTool)
