...
-- Migration: 001_component_geometry_library
-- ============================================================
...
CREATE TABLE mounting_standards (
    ...
    -- e.g. for bolt_circle: {"bolt_size": "M3", "pcd": 16, "count": 4, "bolt_length": 8}
    -- e.g. for pipe: {"nominal_size": "DN25", "od": 33.7, "wall": 3.38, "thread": "BSP"}
...
-- ─── Component Catalogue ────────────────────────────────────
-- Extends the existing `components` table concept.
...
-- If you already have a `components` table, add these columns instead:
-- ALTER TABLE components ADD COLUMN geometry_type_slug TEXT REFERENCES component_geometry_types(slug);
-- ALTER TABLE components ADD COLUMN geometry_params JSONB DEFAULT '{}';
-- ALTER TABLE components ADD COLUMN mounting_points JSONB DEFAULT '[]';
...
CREATE TABLE component_catalogue (
    ...
    -- e.g. {"od": 34.5, "height": 13.5, "shaft_d": 5, "shaft_h": 8, "bolt_pcd": 19, "bolt_count": 4, "bolt_size": 3}
    
    -- Mounting points on THIS part (with positions relative to part origin)
    -- Each point references a mounting_standard and gives its position/orientation
...
CREATE TABLE cad_assemblies (
    ...
    -- e.g. {"frame_size": 5, "prop_size": 5, "battery_cells": 4}
    assembly_params JSONB DEFAULT '{}',
    
    -- Space budget (from interface definition)
    bounding_box JSONB DEFAULT '{}',        -- {"x": 300, "y": 300, "z": 120}
...
CREATE TABLE placed_components (
    ...
    rotation JSONB NOT NULL DEFAULT '{"rx":0,"ry":0,"rz":0}',  -- Euler angles, degrees
    
    -- Assembly metadata
    label TEXT,                             -- e.g. "Front-Left Motor"
    quantity INTEGER DEFAULT 1,
    
    -- Connection to other placed components
    -- e.g. "motor_shaft connects to prop_hub via press_fit_5mm"
...
-- Admin can write geometry types and catalogue
-- (You'd create an admin role or use service key for seeding)
