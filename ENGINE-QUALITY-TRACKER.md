...
- ▶ **RESUME HERE — the LOOP / topology (#135), diagnosed.** The drawings exposed THREE topology defects (all upstream of the drawings — fixing topology auto-fixes every drawing):
  1. **Recirc loop OPEN — 0 edges return to the tanks.** A RAS *must* recirculate. The return-leg DATA exists (`moduleDecomposition.cross_module_grammar_links` has `water_treatment_system → mass_fluid_transport_process [fluid_routing]`, 5 links land on the tanks' module) but the cross-module fluid augmentation (build_universal_scene.py ~L5385 `_AUG_FLUID_MECHANISMS`) routes the return onto a *representative pump*, not the tank SINK, and/or the intra-module pump→tanks leg is missing. FIX: process-RETURN legs should prefer the recirc SINK (the tank/vessel) as the representative; add the intra-module close. **Must stay universal — a once-through plant (e-fuel/CO₂) must add NOTHING.**
