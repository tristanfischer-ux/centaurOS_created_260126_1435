  ...
  if (process.env.ORCHESTRATOR === '1' && engineeringContract) {
    ...
    if (orchResult.ok && orchResult.design) {
      ...
      if (toolsUsedPage?.tools && Array.isArray(toolsUsedPage.tools) && toolsUsedPage.tools.length > 0) {
        ...
        toolOutputsBlock = `\n\nVERIFIED-TOOL OUTPUTS (Build #18 orchestrator — these values were computed by reputable open-source engineering tools and are AUTHORITATIVE. You MUST reference each one explicitly in the relevant module's overview_paragraph_en when discussing that quantity. DO NOT silently override these values with your own estimates):\n${blocks.join('\n\n')}\n\nCRITICAL CONSISTENCY RULES (Build #18p — physics critic catches violations):\n  1. The pack topology values (cell_count, rack_count, cells_per_rack) MUST satisfy rack_count × cells_per_rack = cell_count EXACTLY. No remainder cells allowed.\n  2. The BMS slave count and channel count MUST cover every cell: bms_total_channels ≥ cell_count. If the tool says 418 slaves × 12 channels = 5016 channels for 5008 cells, use those NUMBERS in the design — do NOT invent a smaller BMS sub-system.\n  3. The cold-plate aggregate capacity MUST equal or exceed the system thermal dissipation × 1.25. Use cold_plate_total_capacity_min_kw and cold_plate_per_rack_min_capacity_kw from the tool output.\n  4. Current-carrying components MUST be rated for the tool-derived currents: LCL filter ≥ lcl_filter_rating_a, DC contactor ≥ dc_contactor_rating_a, DC breaker ≥ dc_breaker_rating_a, AC contactor ≥ ac_contactor_rating_a. Do NOT spec components rated below these values — Loop 22's design failed for using a 100A LCL filter where ngspice computed 1443A continuous, a 14× violation.\n  5. The total cell mass plus container plus PCS plus transformer MUST stay within brief.max_mass_kg. If cells alone consume >70% of the budget, split into multiple containers.\n\nFor each tool-sourced quantity above, ensure the module overview text says e.g. "PyBaMM Prada2013 LFP DFN simulation confirms 5008 cells in 16 racks of 313 cells, 4.49 MWh nameplate" rather than just stating the number. The reader needs to know which tool produced which claim so they can independently reproduce it.\n`
