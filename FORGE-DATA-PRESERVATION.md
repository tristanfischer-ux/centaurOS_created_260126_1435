...
1. **All migrations are additive.** New tables are empty on land. Existing columns are untouched. New columns on `foundries` (`tier`, `member_count_cached`, `updated_at`), `audit_log` (`section`, `event`, `actor_user_id`, `actor_specialist`, `payload`), and `foundry_memberships` (`active`, `active_at`, `updated_at`) are nullable or have safe defaults.
2. **SHARED-SCHEMA's `memberships` = existing `foundry_memberships`.** The existing table already has its own backfill (from migration `20260330200000_fix_missing_foundry_memberships.sql`) and its own sync trigger (`profiles_ensure_membership`) that writes to it when `profiles.foundry_id` is set/changed. PR #1 does **not** create a parallel table — it additively extends `foundry_memberships` with the three columns SHARED-SCHEMA §1.2 mandates (`active`, `active_at`, `updated_at`). The role-enum gap (existing 5 CapitalCase values vs. SHARED-SCHEMA's 9 snake_case values) is a separate later PR: expanding the enum cascades to every consumer and is out of PR #1 scope.
3. **`profiles.foundry_id` remains the source of truth** for all existing RLS and `withAuth` resolution. Nothing new reads from `foundry_memberships` in PR #1 that didn't already. Migrating RLS off `profiles.foundry_id` is an explicit later PR, not Phase 1 scope.
4. **`audit_log` dual-write helper** (`src/lib/audit/write.ts`) writes both old column names (`action`, `user_id`, `metadata`) AND new (`event`, `actor_user_id`, `payload`) with the same values. Existing readers of `action`/`user_id`/`metadata` keep working unchanged.
5. **CAD lab route is byte-for-byte untouched.** `/the-forge/cad-lab/*` pages, components, services, and migrations are not modified. The path survives through the Phase 1 cutover rename at PR #12.
6. **Today page is a reskin-plus-frame, not a reset.** All existing signals (`getMorningBriefing`, `getMyDailyPulse`, `getStrategyHealthSummary`, `getUnreadCount`, Cal briefing narrative, onboarding state) continue to flow into the new V3 layout's panel slots. Nothing that currently surfaces on Today disappears.
7. **Sidebar is a chrome swap, not a nav restructure.** PLAN section keeps its legacy 7 items (Strategy/Objectives/Tasks/Review/Reports/Red Team/Knowledge → existing routes). MONEY keeps its legacy 6-item Cash Burn group (no strikethrough pedagogy yet). WORKSHOP's Forge entry is the only flag-aware link.
