/**
 * scripts/lib/orchestrator/tools/pem-membrane-sizing.ts
 *
 * TypeScript wrapper for `pem_membrane_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'pem_membrane_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const pemMembraneSizingTool: Tool<any, any> = {
  id: 'pem-membrane:sizing',
  name: 'PEM Membrane Sizing',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:pem-membrane:sizing',
...
registerTool(pemMembraneSizingTool)
