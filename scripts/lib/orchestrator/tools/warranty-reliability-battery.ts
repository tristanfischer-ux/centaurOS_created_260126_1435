/**
 * scripts/lib/orchestrator/tools/warranty-reliability-battery.ts
 *
 * TypeScript wrapper for `warranty_reliability_battery.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'warranty_reliability_battery.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const warrantyReliabilityBatteryTool: Tool<any, any> = {
  id: 'warranty-reliability:battery',
  name: 'Battery Warranty Reliability',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:warranty-reliability:battery',
...
registerTool(warrantyReliabilityBatteryTool)
