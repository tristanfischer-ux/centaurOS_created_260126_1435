/**
 * scripts/lib/orchestrator/tools/reaction-wheel-sizing.ts
 *
 * TypeScript wrapper for `reaction_wheel_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'reaction_wheel_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const reactionWheelSizingTool: Tool<any, any> = {
  id: 'reaction-wheel:sizing',
  name: 'Reaction Wheel Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'mechanical',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['satellite_smallsat', 'satellite_geo_comsat', 'satellite_interplanetary'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:reaction-wheel:sizing',
...
registerTool(reactionWheelSizingTool)
