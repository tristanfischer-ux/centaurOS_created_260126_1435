/**
 * scripts/lib/orchestrator/tools/delta-v-budget.ts
 *
 * TypeScript wrapper for `delta_v_budget.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'delta_v_budget.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const deltaVBudgetTool: Tool<any, any> = {
  id: 'delta-v-budget:mission',
  name: 'Delta-V Mission Budget',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['satellite_smallsat', 'satellite_geo_comsat', 'satellite_interplanetary', 'propulsion_thruster_product'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:delta-v-budget:mission',
