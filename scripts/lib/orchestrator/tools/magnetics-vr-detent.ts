/**
 * scripts/lib/orchestrator/tools/magnetics-vr-detent.ts
 *
 * TypeScript wrapper for `magnetics_vr_detent.py` — subprocess to repo .venv.
 * Toothed VR + PM detent net holding force (analytic upper bound; FE-calibratable).
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'magnetics_vr_detent.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const magneticsVrDetentTool: Tool<any, any> = {
  id: 'magnetics:vr-detent-force',
  name: 'Magnetics VR Detent Force',
  version: '1.0.0',
...
registerTool(magneticsVrDetentTool)
