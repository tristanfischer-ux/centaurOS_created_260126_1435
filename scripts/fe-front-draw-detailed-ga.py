#!/usr/bin/env python3
"""fe-front-draw-detailed-ga.py — multi-view detailed GA for FE front traction pack.

Consumes parts-manifest.json + twin quantities. Produces:
  drawings/general-arrangement.svg/.png          (3-view overview, denser)
  drawings/ga-detail-interfaces.svg/.png         (connector / QD / HV ICD)
  drawings/ga-detail-section.svg/.png            (axial section story)
  drawings/GA-DETAIL-INDEX.md

INTENT: Blender morphology informs GA — cylinders for motor, shelf for PE,
...
def draw_overview(state: dict, pm: dict, path: Path) -> None:
    ...
    lines.append('<text x="40" y="36" class="t h">GENERAL ARRANGEMENT — Formula E front MGU (detailed)</text>')
    lines.append(
    ...
    lines.append(f'<text x="55" y="{H-95}" class="t s">FRACTIONAL FORGE · ForgeOS · FE Front detailed GA v10</text>')
    lines.append(f'<text x="55" y="{H-75}" class="t xs">Drawing FF-GA-DET-001 · REV P1 · scale ~1:{(1/scale)*25.4/25.4:.1f} schematic · dimensions mm</text>')
...
def main() -> int:
    twin = Path(sys.argv[1] if len(sys.argv) > 1 else "out/formula-e-front-mgu-20260729-1432")
