...
async function main() {
    console.log("Starting standalone Finn pipeline...");
    ...
    await supabase.from('cad_lab_pipeline_runs')
        ...
        .eq('stage', 'finance.cost.seed');
    
    console.log("Calling runFinnCostBackground...");
    try {
        const result = await runFinnCostBackground(projectId, foundryId, userId, "manual");
        console.log("=== RESULT ===");
