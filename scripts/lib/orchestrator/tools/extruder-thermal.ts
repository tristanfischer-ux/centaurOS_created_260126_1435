/**
 * scripts/lib/orchestrator/tools/extruder-thermal.ts
 *
 * TypeScript wrapper for `extruder_thermal.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/3dp
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'extruder_thermal.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const extruderThermalTool: Tool<any, any> = {
  id: 'extruder:thermal',
  name: 'FDM Extruder Thermal Model',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/3dp',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['3d_printer_fdm'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/3dp',
...
registerTool(extruderThermalTool)
