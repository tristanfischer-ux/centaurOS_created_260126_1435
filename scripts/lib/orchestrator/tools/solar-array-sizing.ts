/**
 * scripts/lib/orchestrator/tools/solar-array-sizing.ts
 *
 * TypeScript wrapper for `solar_array_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'solar_array_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const solarArraySizingTool: Tool<any, any> = {
  id: 'solar-array:spacecraft',
  name: 'Spacecraft Solar Array Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'photonics',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['satellite_smallsat', 'satellite_cubesat', 'satellite_geo_comsat', 'satellite_interplanetary'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:solar-array:spacecraft',
...
registerTool(solarArraySizingTool)
