...
- [x] Standalone task product-tagging UI (the Create Task dialog / Task detail panel gets a product `<select>`, mirroring the Edit Objective dialog shipped 2026-04-18). (`9fc39e23`, 2026-04-18 — createTask now reads + validates + persists product_id; CreateTaskDialog accepts optional products[] and renders a "Tag to a product (optional)" select hidden when the foundry has no products; new-tasks/page.tsx fetches products and passes down. Task detail panel product-link chip + other callsites can pick up the same pattern later.)
- [x] Objectives detail: reverse link chip `Linked to Product X` (mirror of the `LinkedProductChip` shipped on CAD Lab; reuse the pattern). (`eadd03ff`, 2026-04-18 — added product_id + product join to ObjectiveWithTasks type + new-objectives/page.tsx SELECT, rendered Package-icon chip in objective-detail-panel header linking to /products/{id}. Shipped alongside the strategic-pillar variant (line 140) and matches the LinkedProductChip pattern from CAD Lab.)
...
- [x] Cash In: surface `probabilityPct` on rows (or stop fetching). (`d9b4475b`, 2026-04-18 — was already rendered as bare "50%" badge; relabelled to "50% likely" + added tooltip explaining the weekly amount on the right is already discounted by the probability factor. Covers Cash Out too since ItemsSection is shared.)
...
- [x] Full-codebase sweep: `grep -rnE "slate-|gray-|bg-white[^a-z]|text-white[^a-z]"` + cleanup. **DONE** 2026-04-18 — violations on `src/app/(platform)/` went from 139 → 0 across 13 batches (67a48768 borders; d27a2fc6 phantom; d414d872 AI prompt; dacef967 STATUS_CONFIG; 2255287c emerald; aaa99680 amber-via-race; 6fbe0cd0 blue; 8621f19b canvas + slate retry; 17451591 StateRow legend; e5a97c19 canvas nodes; 159ebf9b stragglers; df962d7f React Flow handles + palette exceptions in `scripts/check-design-tokens.sh`; bac7ef6f + 2cc4a30f transparency). `./scripts/check-design-tokens.sh src/app/(platform)/` now reports PASSED with 22 advisory warnings only (20 amber text WARNINGS which are intentional and 2 backdrop-blur). Palette files (agent-types.ts, strategy-colors.ts, guild-network.tsx, new-team-form.tsx, marketplace-section-intro.tsx) formally documented as exceptions in the scanner.
- [ ] Products: module-image carryover on `promoteFromCadLab` (per-module URLs, not just hero). *(Deferred: requires schema change — products table has hero_image_url only. Need new JSONB column `module_images` or separate `product_module_images` table, then migration + type regen. Out of scope for single-commit polish.)*
- [x] Products: tooling-investment extraction (buildUnitEconomicsFromEstimates returns `tooling_investment_pence: null` today). (2026-04-18 — extended the loop to sum `tooling_investment_pence` / `tooling_cost_pence` per module and populate `UnitEconomics.tooling_investment_pence` with the total. Stays null if no module reports tooling.)
- [x] Products: Fundability per-row "Apply" consolidation (if re-audit says it's still noisy). *(No change — spot-checked the Fundability panel on 2026-04-18; the per-row Apply buttons are the intentional design and match the "one explicit decision per signal" pattern used elsewhere. Consolidating to a single bulk Apply would hide the per-signal rationale the founder needs to see. Re-audit concluded not noisy.)*
...
| 1.2D | Reports Schedule: generateReport service-role refactor + Resend dispatch | **done** | `053e2686` | pending prod check | Three-part change closes the scheduled-reports loop. report-generator.ts extracted generateReportCore + new generateReportForSchedule (service-role, no withAIGate). report-email.ts added sendReportEmailFromCron (same send, no auth check). cron/scheduled-reports/route.ts now: validates template_id, reads sections/tone/detailLevel from config_json, computes weekly/monthly date range, generates, sends via Resend, logs success or failed, advances next_run_at. First actual email dispatch once the schedule row's next_run_at passes. |
...
| 3.* | Polish sprint | 4/22 done (141, 149, 152, 153) + 18 pending | `75a69a82` (141+149), `d414d872` (152), `67a48768`/`dacef967`/`2255287c`/`aaa99680`/`6fbe0cd0`/`8621f19b`/`17451591`/`e5a97c19`/`159ebf9b`/`df962d7f`/`bac7ef6f`/`2cc4a30f` (153) | pending prod check | Items DONE: 141 (strategy subtitle voice), 149 (Finn cash-out fallback), 152 (briefing personal-commitment framings), 153 (hardcoded-colour sweep — violations on `src/app/(platform)/` cut from 139 → 0 across 13 batches; palette files documented as exceptions in `scripts/check-design-tokens.sh`). 18 other Phase 3 items still open (tasks product-tag UI, objective reverse-chip, Pitch Prep signals + pre-fill, investors pipeline chip, quotes counter, marketplace price-from badge, reports timezone + rate-limit, cash-in probability, P&L NULL handling, products carryover + tooling + consolidation + error toast, UUID validation). |
| 4.1 | /orders | **done** | `f7dcde65` | pending prod check | Page audit was clean on tokens + voice. Only real finding: duplicate "Orders" h1 (page.tsx + orders-view.tsx both rendered one). Removed the duplicate in orders-view, moved Refresh to the filter row with sm:ml-auto, added aria-labels to search input + Refresh button. Tracker question "what did I pay for / what's arriving / who owes me a deliverable" is answerable via Buying tab + status filter. Follow-up could add one-click pivot chips, not blocking. |
