/**
 * scripts/lib/orchestrator/tools/regeneration-energy.ts
 *
 * TypeScript wrapper for `regeneration_energy.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'regeneration_energy.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const regenerationEnergyTool: Tool<any, any> = {
  id: 'dac:regeneration-energy',
  name: 'DAC Regeneration Energy',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/carbon_capture',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['dac'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:dac:regeneration-energy',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/carbon_capture',
...
registerTool(regenerationEnergyTool)
