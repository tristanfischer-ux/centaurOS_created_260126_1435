/**
 * scripts/lib/orchestrator/tools/obstacle-avoidance-sensor.ts
 *
 * TypeScript wrapper for `obstacle_avoidance_sensor.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/drone
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'obstacle_avoidance_sensor.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const obstacleAvoidanceSensorTool: Tool<any, any> = {
  id: 'obstacle-avoidance:sensor',
  name: 'Obstacle Avoidance Sensor Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/drone',
  domain: 'control_systems',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['drone', 'auv'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:obstacle-avoidance:sensor',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/drone',
...
registerTool(obstacleAvoidanceSensorTool)
