...
-- Migration: 20260212300005_seed_tier3_vertical_farming
-- ============================================================
-- Seeds 8 domain-specific geometry types for vertical farming components.
...
def nft_channel(params):
    ...
    return channel
$CADQUERY$,
    '{"length": {"type": "number", "default": 1200.0, "unit": "mm"}, "width": {"type": "number", "default": 100.0, "unit": "mm"}, "height": {"type": "number", "default": 60.0, "unit": "mm"}, "hole_d": {"type": "number", "default": 50.0, "unit": "mm"}, "hole_spacing": {"type": "number", "default": 200.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['aluminium', 'structural', 'hydroponics'],
    '#E0E0E0',
    'NFT (Nutrient Film Technique) grow channel / gutter.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;

-- 2. led_grow_light_bar
INSERT INTO component_geometry_types (slug, name, tier, category, cadquery_code, param_schema, mounting_interfaces, visual_tags, default_colour, description, verified)
VALUES (
    'led_grow_light_bar',
    'LED Grow Light Bar',
...
def led_grow_light_bar(params):
    ...
    return bar
$CADQUERY$,
    '{"length": {"type": "number", "default": 1000.0, "unit": "mm"}, "width": {"type": "number", "default": 40.0, "unit": "mm"}, "led_count": {"type": "integer", "default": 20}}'::jsonb,
    '[]'::jsonb,
    ARRAY['aluminium', 'electrical', 'lighting'],
    '#C0C0C0',
    'Linear LED grow light bar with heatsink and lens array.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def grow_tray(params):
    ...
    return tray
$CADQUERY$,
    '{"length": {"type": "number", "default": 600.0, "unit": "mm"}, "width": {"type": "number", "default": 400.0, "unit": "mm"}, "depth": {"type": "number", "default": 50.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'hydroponics'],
    '#2F4F2F',
    'Shallow flood/drain grow tray with drainage fittings.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def net_pot(params):
    ...
    return cup
$CADQUERY$,
    '{"top_d": {"type": "number", "default": 50.0, "unit": "mm"}, "height": {"type": "number", "default": 50.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'hydroponics'],
    '#1A1A1A',
    'Hydroponic net pot / grow cup with mesh slots for root growth.',
...
def nutrient_reservoir(params):
    ...
    return tank
$CADQUERY$,
    '{"length": {"type": "number", "default": 500.0, "unit": "mm"}, "width": {"type": "number", "default": 300.0, "unit": "mm"}, "height": {"type": "number", "default": 250.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'hdpe', 'hydroponics'],
    '#000080',
    'Rectangular nutrient solution reservoir tank with lid.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def sensor_probe(params):
    ...
    return probe
$CADQUERY$,
    '{"probe_d": {"type": "number", "default": 12.0, "unit": "mm"}, "probe_l": {"type": "number", "default": 120.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'electrical', 'sensor'],
    '#4169E1',
    'pH / EC / dissolved oxygen sensor probe.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def drip_emitter(params):
    ...
    return emitter
$CADQUERY$,
    '{"body_d": {"type": "number", "default": 15.0, "unit": "mm"}, "barb_d": {"type": "number", "default": 6.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'irrigation'],
    '#8B4513',
    'Irrigation drip emitter — inline or button type.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;

-- 8. vertical_tower_section
INSERT INTO component_geometry_types (slug, name, tier, category, cadquery_code, param_schema, mounting_interfaces, visual_tags, default_colour, description, verified)
VALUES (
    'vertical_tower_section',
    'Vertical Grow Tower Section',
    'domain',
    'hydroponics',
$CADQUERY$
def vertical_tower_section(params):
    ...
    return tower
$CADQUERY$,
    '{"od": {"type": "number", "default": 160.0, "unit": "mm"}, "height": {"type": "number", "default": 200.0, "unit": "mm"}, "pocket_count": {"type": "integer", "default": 4}, "pocket_d": {"type": "number", "default": 50.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'structural', 'hydroponics'],
    '#F5F5F5',
    'Stackable vertical grow tower section.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
