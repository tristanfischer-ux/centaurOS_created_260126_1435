...
async function main() {
    console.log("Starting standalone BOM pipeline...");
    ...
    await supabase.from('cad_lab_pipeline_runs')
        ...
        .eq('stage', 'bom.batch.seed');
    
    console.log("Calling runBomGeneratorBackground...");
    try {
        const result = await runBomGeneratorBackground(projectId, foundryId, userId, "manual");
        console.log("=== RESULT ===");
