...
| 3.* | Polish sprint | in progress (3/22 + 153 ~63% done) | `75a69a82`, `67a48768`, `d414d872`, `dacef967`, `2255287c`, `aaa99680`, `6fbe0cd0`, `8621f19b` | pending prod check | Items done: 141, 149, 152. Item 153 ~63% done — violations on `src/app/(platform)/` cut from 139 → 52 across 7 token-swap batches (one earlier batch `d27a2fc6` landed as a phantom empty commit due to the stage-wipe race; its content was re-applied in `8621f19b` alongside canvas `bg-white`/`!bg-white` swaps). Remaining: some transparent `bg-background/XX`, a few residual `bg-white` on canvas nodes (kept reverting mid-commit — needs a calm moment), StateRow legend refactor. |
| 4.1 | /orders | pending | — | — | — |
