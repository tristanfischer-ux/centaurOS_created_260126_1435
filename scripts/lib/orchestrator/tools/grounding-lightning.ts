/**
 * scripts/lib/orchestrator/tools/grounding-lightning.ts
 *
 * TypeScript wrapper for `grounding_lightning.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'grounding_lightning.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const groundingLightningTool: Tool<any, any> = {
  id: 'grounding-lightning:ieee-998',
  ...
  applicable_to(envelope) { return ['bess', 'wind_turbine', 'solar_inverter'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:grounding-lightning:ieee-998',
...
registerTool(groundingLightningTool)
