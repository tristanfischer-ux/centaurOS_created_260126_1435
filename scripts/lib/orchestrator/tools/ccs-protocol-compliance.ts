/**
 * scripts/lib/orchestrator/tools/ccs-protocol-compliance.ts
 *
 * TypeScript wrapper for `ccs_protocol_compliance.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/ev
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'ccs_protocol_compliance.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const ccsProtocolComplianceTool: Tool<any, any> = {
  id: 'ccs-protocol:compliance',
  name: 'CCS Protocol Compliance Lookup',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/ev',
  domain: 'standards',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['ev_charger'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:ccs-protocol:compliance',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/ev',
...
registerTool(ccsProtocolComplianceTool)
