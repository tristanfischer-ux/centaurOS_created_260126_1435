/**
 * scripts/lib/orchestrator/tools/building-envelope-heat-loss.ts
 *
 * TypeScript wrapper for `building_envelope_heat_loss.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/hvac
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'building_envelope_heat_loss.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const buildingEnvelopeHeatLossTool: Tool<any, any> = {
  id: 'building-envelope:heat-loss',
  name: 'Building Envelope Heat Loss',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/hvac',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['heat_pump_residential'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:building-envelope:heat-loss',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/hvac',
...
registerTool(buildingEnvelopeHeatLossTool)
