...
// LEAD-AND-WEAVE (Tristan 2026-06-11): the 3 SYSTEM drawings (general arrangement,
// single-line diagram, P&ID) OPEN Part 2 as the design anchor — the professional
// drawing of what the schematic System Overview describes in words. Reads
// drawing-manifest.json (written by scripts/blender-universal/generate_drawing_set.py
// alongside state.json) and embeds each SYSTEM drawing that landed, full-page
...
function buildSystemDrawingPages(project: string, statePath: string): any[] {
  try {
    ...
    return drawable.map((d: any, i: number) => (
      <Page key={`sysdwg-${d.key}-${i}`} size="A4" orientation="landscape" wrap={false} style={PAGE_STYLE}>
        <PageHeader section="Part 2 · Design drawings" project={project} />
        <Text style={{ fontSize: 20, fontFamily: 'Helvetica-Bold', color: INK, marginBottom: 2 }}>
          {String(d.title || 'Design drawing')}
        </Text>
        <Text style={{ fontSize: 9, color: INK_SOFT, marginBottom: 8, lineHeight: 1.5 }}>{'Projected from the engineering model — the same equipment tags and line numbers as the bill of materials and the schedules.'}</Text>
        <View style={{ flexGrow: 1, alignItems: 'center', justifyContent: 'center', marginVertical: 4 }}><Image src={d.png_abs} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
        </View>
      </Page>
    ))
  } catch {
...
function MinimalDocument({ state, subject, statePath }: { state: any; subject: string; statePath: string }) {
  ...
  return (
    <Document>
      ...
      {/* LEAD-AND-WEAVE (Tristan 2026-06-11): the 3 SYSTEM design-and-construction
          drawings (general arrangement, single-line, P&ID) open Part 2 right after the
          schematic orientation — the professional drawing of the design. Generated by
          generate_drawing_set.py into drawing-manifest.json; absent ⇒ nothing renders. */}
      {buildSystemDrawingPages(project, statePath)}
