/**
 * scripts/lib/orchestrator/tools/fia-net-usable-energy.ts
 *
 * TypeScript wrapper for `fia_net_usable_energy.py`.
 * SOURCE: FIA Formula E Technical Regulations (public PDF) — E_net meter.
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'fia_net_usable_energy.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const FiaNetUsableEnergyTool: Tool<any, any> = {
  id: 'powertrain:fia-net-usable-energy',
  name: 'FIA Net Usable Energy Meter',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    const provenance: Provenance = {
      source: 'tool:powertrain:fia-net-usable-energy',
...
registerTool(FiaNetUsableEnergyTool)
