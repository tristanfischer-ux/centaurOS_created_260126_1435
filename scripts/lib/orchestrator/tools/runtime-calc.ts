/**
 * scripts/lib/orchestrator/tools/runtime-calc.ts
 *
 * TypeScript wrapper for `runtime_calc.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/ups
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'runtime_calc.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const runtimeCalcTool: Tool<any, any> = {
  id: 'runtime:peukert-discharge',
  name: 'Peukert Runtime Calculation',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/ups',
  domain: 'battery',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['ups_inverter', 'e_bike'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:runtime:peukert-discharge',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/ups',
