/**
 * scripts/lib/orchestrator/tools/contactor-geometry.ts
 *
 * TypeScript wrapper for `contactor_geometry.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'contactor_geometry.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const contactorGeometryTool: Tool<any, any> = {
  id: 'dac:contactor-geometry',
  name: 'DAC Contactor Geometry',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:dac:contactor-geometry',
...
registerTool(contactorGeometryTool)
