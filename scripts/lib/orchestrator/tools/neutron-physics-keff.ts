/**
 * scripts/lib/orchestrator/tools/neutron-physics-keff.ts
 *
 * TypeScript wrapper for `neutron_physics_keff.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'neutron_physics_keff.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const neutronPhysicsKeffTool: Tool<any, any> = {
  id: 'smr:neutron-physics-keff',
  name: 'SMR Neutron Physics keff',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:smr:neutron-physics-keff',
...
registerTool(neutronPhysicsKeffTool)
