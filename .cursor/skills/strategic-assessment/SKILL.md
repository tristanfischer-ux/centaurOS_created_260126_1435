...
- User asks for a strategic review or business analysis
- Preparing for investor meetings or board presentations
- Evaluating product readiness for launch or scaling
- Creating an executive summary of current state
- Periodic business health check (quarterly, pre-funding)
...
| Instead Use | When |
|-------------|------|
| `red-team` | Stress-testing specific assumptions or decisions |
| `code-quality` | Technical code review only |
| `design-audit` | UI/UX consistency review |
| `comprehensive-code-review` | Deep technical analysis without business context |
| `bug-fix-workflow` | Investigating specific bugs |
...
| Item | Guideline |
|------|-----------|
| **Dimensions Scored** | Product, Business Model, Operations, Market Position |
| **Grading Scale** | A-F (A = excellent, F = failing) |
| **Risk Severities** | HIGH (2 weeks), MEDIUM (30 days), LOW (90 days) |
| **Action Plan Format** | Week/Month + Action + Owner (role) |
| **Confidence Level** | X/10 with explanation |
| **Output Format** | Markdown document with tables and sections |
| **Data Sources** | Codebase metrics, database schema, feature inventory |
| **Follow-up** | Red team the findings to stress-test conclusions |

---

## Related Skills

- **`red-team`** - Run after assessment to stress-test findings and recommendations
- **`design-audit`** - Complements assessment with detailed UI/UX analysis
