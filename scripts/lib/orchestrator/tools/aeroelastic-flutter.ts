/**
 * scripts/lib/orchestrator/tools/aeroelastic-flutter.ts
 *
 * TypeScript wrapper for `aeroelastic_flutter.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'aeroelastic_flutter.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const aeroelasticFlutterTool: Tool<any, any> = {
  id: 'aeroelastic-flutter:wing',
  name: 'Aeroelastic Flutter Wing Analysis',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:aeroelastic-flutter:wing',
...
registerTool(aeroelasticFlutterTool)
