/**
 * scripts/lib/orchestrator/tools/decay-heat-loca.ts
 *
 * TypeScript wrapper for `decay_heat_loca.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'decay_heat_loca.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const decayHeatLocaTool: Tool<any, any> = {
  id: 'smr:decay-heat-loca',
  name: 'SMR Decay Heat LOCA',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:smr:decay-heat-loca',
...
registerTool(decayHeatLocaTool)
