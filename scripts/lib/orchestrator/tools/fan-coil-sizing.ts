/**
 * scripts/lib/orchestrator/tools/fan-coil-sizing.ts
 *
 * TypeScript wrapper for `fan_coil_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'fan_coil_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const fanCoilSizingTool: Tool<any, any> = {
  id: 'fan-coil:sizing',
  name: 'Fan Coil Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/hvac',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['bess', 'vertical_farm', 'heat_pump_residential', 'bioreactor', 'edge_ai', 'ups_inverter', 'ventilator'].includes(envelope.class) },
...
registerTool(fanCoilSizingTool)
