            ...
            try {
                if (liveThreadId) {
                    ...
                    if (completeResult.ok) {
                        ...
                        // Poll for cover/audio URLs generated by after() block.
                        // after() runs asynchronously so assets aren't ready yet.
                        const tid = liveThreadId
                        const pollForAssets = async () => {
                            for (let i = 0; i < 12; i++) {
                                await new Promise(r => setTimeout(r, 5_000))
                                try {
                                    const urls = await refreshSessionAssetUrls(tid)
                                    if (urls.coverUrl && !generatedCoverUrlRef.current) {
                                        setGeneratedCoverUrl(urls.coverUrl)
                                        generatedCoverUrlRef.current = urls.coverUrl
                                        setGraphicStatus("done")
                                    }
                                    if (urls.audioUrl && !generatedAudioUrlRef.current) {
                                        setGeneratedAudioUrl(urls.audioUrl)
                                        generatedAudioUrlRef.current = urls.audioUrl
                                        setAudioStatus("done")
                                    }
                                    if (urls.coverUrl && urls.audioUrl) break
