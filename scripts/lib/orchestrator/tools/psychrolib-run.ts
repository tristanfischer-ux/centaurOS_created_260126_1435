/**
 * scripts/lib/orchestrator/tools/psychrolib-run.ts
 *
 * TypeScript wrapper for `psychrolib_run.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'psychrolib_run.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const psychrolibRunTool: Tool<any, any> = {
  id: 'psychrolib:humid-air',
  name: 'psychrolib Humid Air Calcs',
  version: '2.5.0',
  license: 'MIT',
  source_url: 'github.com/psychrometrics/psychrolib',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4', psychrolib: '2.5.0' },
  applicable_to(envelope) { return ['vertical_farm', 'haps', 'heat_pump_residential', 'bioreactor', 'ventilator', 'h2_electrolyser'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:psychrolib:humid-air',
...
registerTool(psychrolibRunTool)
