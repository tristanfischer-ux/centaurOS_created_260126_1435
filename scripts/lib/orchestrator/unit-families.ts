/**
 * scripts/lib/orchestrator/unit-families.ts
 *
 * UNIT FAMILIES + SCALE-METRIC FINDER — LEAF MODULE (zero orchestrator
 * imports beyond the type-only `./types`, which itself imports nothing).
 *
 * WHY A LEAF (2026-06-10, tracker #19): commit 8df6d96a1 (E1 universal
 * envelope vector) created the import cycle
 *   constraint-normaliser → envelope → envelope-vector → constraint-normaliser
 * and envelope-vector.ts accesses ENERGY_KWH at MODULE level (its
 * FAMILY_PROBES table), so any consumer that loaded constraint-normaliser
 * first crashed at import time with
 *   "ReferenceError: Cannot access 'ENERGY_KWH' before initialization".
 * The shared unit-family constants + pure helpers now live HERE; the three
 * former cycle participants all import from this leaf, so no module-level
 * initialiser ever runs against a half-initialised module again. The
 * remaining envelope ↔ envelope-vector cycle is function-body-only
 * (documented at envelope.ts:22-24) and safe.
 *
 * constraint-normaliser.ts RE-EXPORTS everything here under the original
 * names, so existing consumers (`serial-design-chain-v2.tsx`,
 * `regression-harness.tsx`, detectors) need no changes.
 *
 * CONTENT moved verbatim from constraint-normaliser.ts (council-verdict
 * provenance 2026-05-23 retained there — see that file's header).
 */
...
export function normUnit(raw: string | null | undefined): string {
  if (raw == null) return ''
...
export function isUsableValue(v: unknown): v is number {
...
export function captureNumber(re: RegExp, text: string): number | null {
 ...
 *  than throwing. */
export function asArray<T>(maybeArr: unknown): ReadonlyArray<T> {
