/**
 * scripts/lib/orchestrator/tools/nutrient-solution-chemistry.ts
 *
 * TypeScript wrapper for `nutrient_solution_chemistry.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'nutrient_solution_chemistry.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const nutrientSolutionChemistryTool: Tool<any, any> = {
  id: 'nutrient-solution:chemistry',
  name: 'Nutrient Solution Chemistry',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:nutrient-solution:chemistry',
...
registerTool(nutrientSolutionChemistryTool)
