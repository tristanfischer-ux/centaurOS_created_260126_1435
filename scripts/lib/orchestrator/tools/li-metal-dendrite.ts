/**
 * scripts/lib/orchestrator/tools/li-metal-dendrite.ts
 *
 * TypeScript wrapper for `li_metal_dendrite.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'li_metal_dendrite.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const liMetalDendriteTool: Tool<any, any> = {
  id: 'li-metal-dendrite:monroe-newman',
  name: 'Li Metal Dendrite Monroe-Newman',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:li-metal-dendrite:monroe-newman',
...
registerTool(liMetalDendriteTool)
