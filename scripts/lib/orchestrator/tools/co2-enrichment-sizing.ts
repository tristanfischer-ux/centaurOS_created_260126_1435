/**
 * scripts/lib/orchestrator/tools/co2-enrichment-sizing.ts
 *
 * TypeScript wrapper for `co2_enrichment_sizing.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/agriculture
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'co2_enrichment_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const co2EnrichmentSizingTool: Tool<any, any> = {
  id: 'co2-enrichment:sizing',
  name: 'CO₂ Enrichment Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/agriculture',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['vertical_farm', 'bioreactor'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:co2-enrichment:sizing',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/agriculture',
...
registerTool(co2EnrichmentSizingTool)
