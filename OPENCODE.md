...
## LLM MODEL MAP (which model runs each non-deterministic stage)
**`docs/MODELS.md`** — per-stage map + benchmark-grounded upgrade audit. Registry (change models here): `src/lib/pdf-engine-v2/lib/openrouter-models.ts`. Pipeline is Anthropic-free (all OpenRouter). Headlines: brief gen/expand = `gemini-3.1-pro`; emit/judge/self-audit = `grok-4.3`; honest review = `mimo-v2.5-pro`. The ONE real chain mismatch: physics critic runs on `gemini-3.5-flash` (a flash tier for physics) — recommend `gemini-3.1-pro`. Chain is already current-gen; no newer tier to jump to per the May-2026 benchmark — fixes are role-fit, not version bumps. Validate any swap with a full chain + scorecard-gate.
