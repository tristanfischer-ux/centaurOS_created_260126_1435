...
export interface CadLabProjectData {
  ...
  /** When the design brief was locked. NULL = editable. */
  briefLockedAt: string | null
  /** Soft-archive timestamp. Non-null = hidden from default workspace list. */
  archivedAt: string | null
  /** Source project ID if this is a fork. */
...
> {
  return withAuth(async ({ supabase }) => {
    // Default workspace view hides archived rows. A future page can expose
    // them via ?view=archived by passing a flag to a sibling listArchivedCadLabProjects.
  ...
  return withAuth(async ({ supabase }) => {
    ...
    return {
      project: {
        ...
        briefLockedAt: (project as unknown as { brief_locked_at?: string | null }).brief_locked_at ?? null,
        archivedAt: (project as unknown as { archived_at?: string | null }).archived_at ?? null,
        forkedFromId: (project as unknown as { forked_from_id?: string | null }).forked_from_id ?? null,
      },
    }
  })
}
