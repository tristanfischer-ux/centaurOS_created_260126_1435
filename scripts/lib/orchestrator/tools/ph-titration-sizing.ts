/**
 * scripts/lib/orchestrator/tools/ph-titration-sizing.ts
 *
 * TypeScript wrapper for `ph_titration_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'ph_titration_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const phTitrationSizingTool: Tool<any, any> = {
  id: 'ph-titration:sizing',
  name: 'pH Titration Sizing',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:ph-titration:sizing',
...
registerTool(phTitrationSizingTool)
