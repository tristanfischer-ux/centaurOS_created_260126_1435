/**
 * scripts/lib/orchestrator/tools/gust-response-atmospheric.ts
 *
 * TypeScript wrapper for `gust_response_atmospheric.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'gust_response_atmospheric.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const gustResponseAtmosphericTool: Tool<any, any> = {
  id: 'gust-response:haps-atmospheric',
  name: 'Gust Response Atmospheric HAPS',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/aero',
  domain: 'aero',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['haps'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:gust-response:haps-atmospheric',
...
registerTool(gustResponseAtmosphericTool)
