-- Re-declare match_marketplace_listings_v2 as VOLATILE so SET LOCAL is allowed
-- (STABLE functions can't issue SET statements — pgvector restriction).
...
COMMENT ON FUNCTION match_marketplace_listings_v2 IS
  ...
  'VOLATILE so SET LOCAL hnsw.ef_search=1000 is legal — HNSW default 40 was '
  'silently capping match_count. Aligns with Forge Capital Dashboard for parity.';
