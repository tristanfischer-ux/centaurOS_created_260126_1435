/**
 * scripts/lib/orchestrator/tools/sonar-acoustic.ts
 *
 * TypeScript wrapper for `sonar_acoustic.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'sonar_acoustic.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const sonarAcousticTool: Tool<any, any> = {
  id: 'sonar:acoustic-attenuation',
  name: 'Sonar Acoustic Attenuation',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:sonar:acoustic-attenuation',
...
registerTool(sonarAcousticTool)
