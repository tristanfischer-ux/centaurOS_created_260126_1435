/**
 * scripts/lib/orchestrator/tools/plant-growth.ts
 *
 * TypeScript wrapper for `plant_growth.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/biology
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'plant_growth.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const plantGrowthTool: Tool<any, any> = {
  id: 'plant-growth:yield',
  name: 'Plant Growth Yield Model',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/biology',
  domain: 'biochemistry',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:plant-growth:yield',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/biology',
