/**
 * scripts/lib/orchestrator/tools/flow-compliance.ts
 *
 * TypeScript wrapper for `flow_compliance.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'flow_compliance.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const flowComplianceTool: Tool<any, any> = {
  id: 'flow-compliance:respiratory',
  name: 'Flow + Compliance Respiratory',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:flow-compliance:respiratory',
...
registerTool(flowComplianceTool)
