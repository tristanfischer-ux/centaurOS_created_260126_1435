/**
 * scripts/lib/orchestrator/tools/dehumidification-calc.ts
 *
 * TypeScript wrapper for `dehumidification_calc.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'dehumidification_calc.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const dehumidificationCalcTool: Tool<any, any> = {
  id: 'dehumidification:sizing',
  ...
  applicable_to(envelope) { return ['vertical_farm', 'heat_pump_residential', 'ventilator'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:dehumidification:sizing',
...
registerTool(dehumidificationCalcTool)
