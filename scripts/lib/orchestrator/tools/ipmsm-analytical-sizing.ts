/**
 * scripts/lib/orchestrator/tools/ipmsm-analytical-sizing.ts
 *
 * TypeScript wrapper for `ipmsm_analytical_sizing.py` — subprocess to repo .venv.
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'ipmsm_analytical_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const IpmsmAnalyticalSizingTool: Tool<any, any> = {
  id: 'motor:ipmsm-analytical-sizing',
  name: 'Radial-Flux IPMSM Analytical First Sizing',
  version: '1.0.0',
...
registerTool(IpmsmAnalyticalSizingTool)
