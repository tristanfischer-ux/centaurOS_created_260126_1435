/**
 * scripts/lib/orchestrator/tools/dilution-fridge-cooling-power.ts
 *
 * TypeScript wrapper for `dilution_fridge_cooling_power.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'dilution_fridge_cooling_power.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const dilutionFridgeCoolingPowerTool: Tool<any, any> = {
  id: 'dilution-fridge:cooling-power',
  name: 'Dilution Fridge Cooling Power',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/cryogenics',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['cryostat', 'quantum_computer'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:dilution-fridge:cooling-power',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/cryogenics',
...
registerTool(dilutionFridgeCoolingPowerTool)
