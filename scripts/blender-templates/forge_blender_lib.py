...
def init_scene():
    ...
    # Standard view transform — pure white world renders as pure white. AgX/Filmic
    # crush highlights for photoreal; we want diagrammatic output.
...
def add_lights(target_centre=(0, 0, 0), sun_energy=3.0, fill_energy=60, fill_size=2.5):
    """Standard 2-light setup — sun at 55°/20°/35° + area fill from above."""
    ...
    bpy.ops.object.light_add(type="AREA", location=(target_centre[0], target_centre[1], target_centre[2] + 2.5))
...
def make_mat(name, rgb, metallic=0.0, roughness=0.55, alpha=1.0):
    """Principled BSDF material with optional alpha."""
...
def run_render_pipeline(out_dir, module_objects, structure_module_id="structure_containment",
    ...
    for module_id, mod_objs in module_objects.items():
        ...
        scene.render.filepath = str(out_dir / f"module-{module_id}.png")
        bpy.ops.render.render(write_still=True)
        print(f"[forge] module-{module_id}.png")
