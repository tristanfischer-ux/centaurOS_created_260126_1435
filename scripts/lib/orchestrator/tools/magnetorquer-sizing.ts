/**
 * scripts/lib/orchestrator/tools/magnetorquer-sizing.ts
 *
 * TypeScript wrapper for `magnetorquer_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'magnetorquer_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const magnetorquerSizingTool: Tool<any, any> = {
  id: 'magnetorquer:sizing',
  name: 'Magnetorquer Sizing',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:magnetorquer:sizing',
...
registerTool(magnetorquerSizingTool)
