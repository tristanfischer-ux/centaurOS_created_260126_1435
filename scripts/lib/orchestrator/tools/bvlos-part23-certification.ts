/**
 * scripts/lib/orchestrator/tools/bvlos-part23-certification.ts
 *
 * TypeScript wrapper for `bvlos_part23_certification.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'bvlos_part23_certification.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const bvlosPart23CertificationTool: Tool<any, any> = {
  id: 'bvlos-part23:certification',
  name: 'BVLOS Part 23 Certification',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:bvlos-part23:certification',
...
registerTool(bvlosPart23CertificationTool)
