...
-- Migration: 20260212300002_seed_tier3_bsf_brine_eps
-- ============================================================
-- Seeds 18 domain-specific geometry types:
--   BSF Breeding (5), Brine Mining (7), EPS Architectural (6)

-- ============================================================
-- BLACK SOLDIER FLY (BSF) BREEDING — 5 components
-- ============================================================
...
def breeding_cage(params):
    ...
    return frame
$CADQUERY$,
    '{"length": {"type": "number", "default": 600.0, "unit": "mm"}, "width": {"type": "number", "default": 400.0, "unit": "mm"}, "height": {"type": "number", "default": 500.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['metal', 'structural', 'mesh'],
    '#808080',
    'Mesh breeding cage / enclosure — frame with mesh panels.',
    TRUE
...
def egg_collection_plate(params):
    ...
    return base
$CADQUERY$,
    '{"length": {"type": "number", "default": 200.0, "unit": "mm"}, "corrugations": {"type": "integer", "default": 8}}'::jsonb,
    '[]'::jsonb,
    ARRAY['wood', 'corrugated'],
    '#8B4513',
    'Corrugated egg collection plate for BSF oviposition.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def larvae_rearing_tray(params):
    ...
    return tray
$CADQUERY$,
    '{"length": {"type": "number", "default": 600.0, "unit": "mm"}, "width": {"type": "number", "default": 400.0, "unit": "mm"}, "depth": {"type": "number", "default": 80.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['plastic', 'hdpe'],
    '#696969',
    'Shallow rearing tray with drainage holes and handles.',
...
def ventilation_louvre(params):
    ...
    return panel
$CADQUERY$,
    '{"width": {"type": "number", "default": 200.0, "unit": "mm"}, "height": {"type": "number", "default": 150.0, "unit": "mm"}, "slat_count": {"type": "integer", "default": 8}}'::jsonb,
    '[]'::jsonb,
    ARRAY['aluminium', 'structural'],
    '#C0C0C0',
    'Angled slat ventilation panel.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def harvesting_screen(params):
    ...
    return frame
$CADQUERY$,
    '{"length": {"type": "number", "default": 500.0, "unit": "mm"}, "width": {"type": "number", "default": 300.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['metal', 'mesh'],
    '#A0A0A0',
    'Sieve / harvesting screen — mesh grid in a frame.',
...
def pressure_vessel(params):
    ...
    return vessel
$CADQUERY$,
    '{"od": {"type": "number", "default": 600.0, "unit": "mm"}, "length": {"type": "number", "default": 2000.0, "unit": "mm"}, "wall": {"type": "number", "default": 8.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['steel', 'pressure', 'vessel'],
    '#708090',
    'Cylindrical pressure vessel with flanged ends and nozzle.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def hydrocyclone(params):
    ...
    return body
$CADQUERY$,
    '{"body_d": {"type": "number", "default": 300.0, "unit": "mm"}, "body_h": {"type": "number", "default": 400.0, "unit": "mm"}, "cone_h": {"type": "number", "default": 600.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['steel', 'process'],
    '#4682B4',
    'Conical hydrocyclone separator — cylindrical body tapering to cone.',
...
def crystalliser_vessel(params):
    ...
    return vessel
$CADQUERY$,
    '{"od": {"type": "number", "default": 800.0, "unit": "mm"}, "cylinder_height": {"type": "number", "default": 1500.0, "unit": "mm"}, "cone_height": {"type": "number", "default": 600.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['steel', 'process', 'vessel'],
    '#708090',
    'Crystalliser — tall cylinder with conical bottom and draft tube.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def filter_press_plate(params):
    ...
    return plate
$CADQUERY$,
    '{"size": {"type": "number", "default": 400.0, "unit": "mm"}, "thickness": {"type": "number", "default": 40.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['polypropylene', 'filtration'],
    '#4B0082',
    'Single filter press plate with recess and feed/filtrate ports.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def product_hopper(params):
    ...
    return hopper
$CADQUERY$,
    '{"top_length": {"type": "number", "default": 600.0, "unit": "mm"}, "top_width": {"type": "number", "default": 400.0, "unit": "mm"}, "height": {"type": "number", "default": 500.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['steel', 'structural'],
    '#A0A0A0',
    'Product bin / hopper with sloped bottom for discharge.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
def pipe_flange(params):
    ...
    return disc
$CADQUERY$,
    '{"pipe_od": {"type": "number", "default": 60.3, "unit": "mm"}, "flange_od": {"type": "number", "default": 150.0, "unit": "mm"}, "bolt_count": {"type": "integer", "default": 8}, "blind": {"type": "boolean", "default": false}}'::jsonb,
    '[]'::jsonb,
    ARRAY['steel', 'piping'],
    '#A0A0A0',
    'Pipe flange — slip-on or blind, with bolt holes.',
...
$CADQUERY$,
    '{"body_d": {"type": "number", "default": 80.0, "unit": "mm"}, "port_d": {"type": "number", "default": 50.0, "unit": "mm"}, "actuator_d": {"type": "number", "default": 60.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['steel', 'cast', 'valve'],
    '#696969',
    'Globe control valve body with actuator housing.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
-- ============================================================
-- EPS ARCHITECTURAL PARTS — 6 components
-- ============================================================
...
$CADQUERY$,
    '{"length": {"type": "number", "default": 2400.0, "unit": "mm"}, "width": {"type": "number", "default": 1200.0, "unit": "mm"}, "height": {"type": "number", "default": 600.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['foam', 'eps', 'insulation'],
    '#F5F5F5',
    'Raw EPS block / billet for CNC or hot-wire cutting.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
$CADQUERY$,
    '{"throat_width": {"type": "number", "default": 600.0, "unit": "mm"}, "throat_depth": {"type": "number", "default": 400.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['metal', 'tooling'],
    '#FF4500',
    'Hot wire cutter U-frame for EPS profiling.',
...
def reinforcing_mesh_panel(params):
    ...
    return mesh
$CADQUERY$,
    '{"length": {"type": "number", "default": 1000.0, "unit": "mm"}, "width": {"type": "number", "default": 500.0, "unit": "mm"}, "mesh_spacing": {"type": "number", "default": 50.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['fibreglass', 'mesh'],
    '#FFD700',
    'Fibreglass or steel reinforcing mesh for EPS render systems.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
$CADQUERY$,
    '{"leg_a": {"type": "number", "default": 60.0, "unit": "mm"}, "leg_b": {"type": "number", "default": 60.0, "unit": "mm"}, "length": {"type": "number", "default": 2400.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['pvc', 'aluminium', 'trim'],
    '#C0C0C0',
    'L-shaped corner bead / edge profile for EPS cladding systems.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
...
$CADQUERY$,
    '{"disc_diameter": {"type": "number", "default": 60.0, "unit": "mm"}, "shaft_length": {"type": "number", "default": 100.0, "unit": "mm"}}'::jsonb,
    '[]'::jsonb,
    ARRAY['nylon', 'fixing'],
    '#FFA500',
    'Mechanical EPS fixing anchor — disc head with expansion shaft.',
...
def surface_texture_panel(params):
    ...
    return panel
$CADQUERY$,
    '{"length": {"type": "number", "default": 600.0, "unit": "mm"}, "width": {"type": "number", "default": 400.0, "unit": "mm"}, "thickness": {"type": "number", "default": 50.0, "unit": "mm"}, "pattern": {"type": "string", "default": "ashlar", "enum": ["ashlar", "rusticated", "smooth"]}}'::jsonb,
    '[]'::jsonb,
    ARRAY['eps', 'facade', 'architectural'],
    '#DEB887',
    'Decorative EPS facade panel with surface texture pattern.',
    TRUE
) ON CONFLICT (slug) DO NOTHING;
