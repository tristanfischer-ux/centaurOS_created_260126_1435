 ...
 * Handles routing between marketing domain (fractionalforge.io) and app domain (forgeos.io)
 */

export const MARKETING_DOMAIN = process.env.NEXT_PUBLIC_MARKETING_DOMAIN || 'https://fractionalforge.io'
export const APP_DOMAIN = process.env.NEXT_PUBLIC_APP_DOMAIN || 'https://forgeos.io'
...
export function isMarketingDomain(hostname: string): boolean {
  return hostname.includes('fractionalforge.io')
}
...
export function isAppDomain(hostname: string): boolean {
  return hostname.includes('forgeos.io')
}
