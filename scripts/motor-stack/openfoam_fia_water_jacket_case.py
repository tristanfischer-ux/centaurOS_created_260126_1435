#!/usr/bin/env python3
"""FIA-bound OpenFOAM water-jacket screening case for the Formula E front kit.
...
binds the ``motor_water_jacket_helical`` family channel cross-section, and runs
a simplified rectangular-duct screen — not conjugate heat transfer of the full
helical jacket STEP.

Status stays PARTIAL (or OPEN if the solve fails honestly); ``ship_ok`` is
always false.  Winding temperatures are not claimed closed.
"""
...
RUNNER_SH = SCRIPT_DIR / "openfoam_fia_water_jacket_case.sh"
OPENFOAM_IMAGE = "microfluidica/openfoam:14"

# Family defaults from motor_water_jacket_helical / hydraulics helper.
DEFAULT_CHANNEL_WIDTH_MM = 8.0
DEFAULT_CHANNEL_DEPTH_MM = 3.5
DEFAULT_HELIX_TURNS = 5.0
DEFAULT_HOUSING_OD_MM = 176.7
DEFAULT_JACKET_ID_MM = 164.7
DEFAULT_AXIAL_LENGTH_MM = 140.5
DEFAULT_OUTER_SHELL_MM = 1.25
DEFAULT_END_MARGIN_MM = 8.0
...
class FiaWaterJacketOpenFoamError(RuntimeError):
    """Raised when twin binding or OpenFOAM evidence is incomplete."""


@dataclass(frozen=True)
class TwinInputs:
    """Selected twin quantities that control this water-jacket screen."""
    ...
    jacket_inner_diameter_mm: float
...
class ChannelGeometry:
    """Simplified rectangular duct representing one helical turn share."""
...
class PressureDropResult:
    ...
    solver_delta_p_per_turn_pa: float | None
...
def _number(
    ...
    *,
    ...
    if default is not None:
        return default
    raise FiaWaterJacketOpenFoamError(
...
def inputs_from_sections(quantities: Mapping[str, Any]) -> TwinInputs:
    ...
    INTENT: Jacket hydraulics must answer pressure-drop / velocity screening at
    the FIA coolant point using twin flow + family channel dims — never a
    ...
    except FiaWaterJacketOpenFoamError:
        twin_rho = None
    return TwinInputs(
        ...
        channel_width_mm=_number(
            quantities,
            ("jacket_channel_width_mm", "water_jacket_channel_width_mm"),
            default=DEFAULT_CHANNEL_WIDTH_MM,
        ),
        channel_depth_mm=_number(
            quantities,
            ("jacket_channel_depth_mm", "water_jacket_channel_depth_mm"),
            default=DEFAULT_CHANNEL_DEPTH_MM,
        ),
        helix_turns=_number(
            quantities,
            ("jacket_helix_turns", "water_jacket_helix_turns"),
            default=DEFAULT_HELIX_TURNS,
        ),
        housing_outer_diameter_mm=_number(
            quantities,
            ("fpk_housing_od_mm", "housing_outer_diameter_mm"),
            default=DEFAULT_HOUSING_OD_MM,
        ),
        jacket_inner_diameter_mm=_number(
            quantities,
            ("fpk_stator_od_mm", "jacket_inner_diameter_mm"),
            default=DEFAULT_JACKET_ID_MM,
        ),
        axial_length_mm=_number(
            quantities,
            ("fpk_housing_len_mm", "jacket_axial_length_mm"),
            default=DEFAULT_AXIAL_LENGTH_MM,
        ),
        outer_shell_mm=_number(
            quantities,
            ("jacket_outer_shell_mm",),
            default=DEFAULT_OUTER_SHELL_MM,
        ),
        end_margin_mm=_number(
            quantities,
            ("jacket_end_margin_mm",),
            default=DEFAULT_END_MARGIN_MM,
        ),
...
def load_twin_inputs(state_path: Path) -> tuple[TwinInputs, str]:
    ...
    if not state_path.is_file():
        raise FiaWaterJacketOpenFoamError(f"Twin state not found: {state_path}")
    ...
    raise FiaWaterJacketOpenFoamError(f"{last_error}; rerun on a stable stamp")
...
def derive_channel_geometry(inputs: TwinInputs) -> ChannelGeometry:
    """Map twin + family dimensions onto a one-turn rectangular duct.

    DECISION: Helical jacket is a series channel — full volumetric flow through
    the family rectangular section.  OpenFOAM models one developed helical turn;
    full-path Δp is estimated as turns × per-turn (manifold / port losses OPEN).
    """

    if inputs.helix_turns < 2.0:
        raise FiaWaterJacketOpenFoamError("helix_turns must be at least 2")
    if inputs.jacket_inner_diameter_mm >= inputs.housing_outer_diameter_mm:
        raise FiaWaterJacketOpenFoamError(
            "jacket_inner_diameter must be less than housing_outer_diameter"
        )
    radial_wall = (
        inputs.housing_outer_diameter_mm - inputs.jacket_inner_diameter_mm
    ) / 2.0
    if inputs.channel_depth_mm + inputs.outer_shell_mm >= radial_wall:
        raise FiaWaterJacketOpenFoamError(
            "channel_depth + outer_shell exceeds annular wall"
        )
    inner_bridge = radial_wall - inputs.channel_depth_mm - inputs.outer_shell_mm
    r_mid_mm = (
        inputs.jacket_inner_diameter_mm / 2.0
        + inner_bridge
        + inputs.channel_depth_mm / 2.0
    )
    if 2.0 * inputs.end_margin_mm >= inputs.axial_length_mm:
        raise FiaWaterJacketOpenFoamError("end_margin leaves no active helical length")
    active_length_mm = inputs.axial_length_mm - 2.0 * inputs.end_margin_mm
    pitch_mm = active_length_mm / inputs.helix_turns
    one_turn_mm = math.hypot(2.0 * math.pi * r_mid_mm, pitch_mm)
    full_path_mm = one_turn_mm * inputs.helix_turns

    width_m = inputs.channel_width_mm / 1000.0
    depth_m = inputs.channel_depth_mm / 1000.0
    duct_length_m = one_turn_mm / 1000.0
    area = width_m * depth_m
    if area <= 0.0:
        raise FiaWaterJacketOpenFoamError("Channel cross-section must be positive")
    ...
    return ChannelGeometry(
        ...
        helix_turns=inputs.helix_turns,
        channel_mean_diameter_m=2.0 * r_mid_mm / 1000.0,
        helix_pitch_m=pitch_mm / 1000.0,
        flow_split="full_flow_single_helical_channel",
        ...
        estimated_full_path_length_m=full_path_mm / 1000.0,
    )
...
def compute_pressure_drop(
    ...
    *,
    ...
    dp_lam, dp_bla, f_lam, f_bla = analytical_darcy_delta_p_pa(
        geometry, length_m=geometry.duct_length_m
    )
    ...
    solver_per_turn: float | None = None
    ...
    if solver_delta_p_kinematic is not None and math.isfinite(
        ...
        solver_per_turn = solver_pa
        full_est = solver_per_turn * float(geometry.helix_turns)
    # DECISION: Prefer solver per-turn Δp when available; else Blasius if
    # transitional/turbulent, else laminar Darcy.
    if solver_per_turn is not None and solver_per_turn > 0.0:
        headline = full_est if full_est is not None else solver_per_turn
        source = "openfoam_laminar_n_times_per_turn"
    ...
    return PressureDropResult(
        ...
        solver_delta_p_per_turn_pa=solver_per_turn,
...
def run_openfoam(case_dir: Path) -> SolverRun:
    ...
    if not RUNNER_SH.is_file():
        raise FiaWaterJacketOpenFoamError(f"Runner missing: {RUNNER_SH}")
...
def extract_inlet_pressure_kinematic(case_dir: Path, latest_time: str) -> float:
    ...
    ok, runtime = _toolchain_available()
    if not ok:
        raise FiaWaterJacketOpenFoamError(f"Cannot post-process: {runtime}")
...
def build_artifact(
    *,
    ...
    """Assemble the honest, permanently non-release water-jacket artefact."""

    if status not in {"PARTIAL", "OPEN"}:
        raise FiaWaterJacketOpenFoamError("status must be PARTIAL or OPEN")
    return {
        "schema": SCHEMA,
        "status": status,
        "ship_ok": False,
        ...
        "model_assumptions": [
            ...
            "Duct length = one helical turn developed length.",
            "Inlet velocity from full 12 L/min / channel area (series helix).",
            f"Water-like ν = {WATER_NU_60C_M2_S:.3e} m²/s at ~60 °C (documented).",
            "Laminar steady SIMPLE — not a mesh-converged turbulent CHT study.",
            "Full-path Δp ≈ n_turns × per-turn solver Δp; port/manifold losses OPEN.",
            "No solid domain, no winding temperatures, no boiling assessment.",
        ],
        "geometry_provenance": {
            "controlling_dimensions": (
                "orchestratorContract coolant_* + fpk_housing_* / fpk_stator_od "
                "with motor_water_jacket_helical family channel defaults"
            ),
            "full_helical_step_meshed": False,
            "lucid_or_proprietary_cad_used": False,
            "statement": (
                "Twin-bound rectangular-channel screen using family channel dims "
                "and FIA coolant point; not supplier water-jacket CAD."
            ),
        },
        "fia_question": (
            f"What duct velocity / pressure drop appear at "
            f"{inputs.coolant_flow_l_min:g} L/min / {inputs.coolant_inlet_c:g} °C "
            "on the motor_water_jacket_helical channel section?"
        ),
        "winding_temperatures": {
            "status": "OPEN",
            "statement": (
                "This case does not close winding temperatures or conjugate heat "
                "transfer. Pressure-drop and velocity screening only."
            ),
        },
        "conjugate_heat_transfer": {
            "status": "OPEN",
            "statement": "Full helical jacket STEP CHT remains OPEN.",
        },
        "flow_bench_correlation": {
            "status": "OPEN",
            "statement": "No measured jacket Δp / flow-bench correlation yet.",
        },
        "honesty_notes": [
            "PARTIAL means a twin-bound OpenFOAM duct screen exists — not release.",
            "ship_ok is permanently false for this artefact schema.",
            "Do not claim winding ΔT closed from this pressure/velocity screen.",
...
def _synthetic_quantities() -> dict[str, Any]:
    ...
    return {
        ...
        "jacket_channel_width_mm": {"value": 8.0, "unit": "mm"},
        "jacket_channel_depth_mm": {"value": 3.5, "unit": "mm"},
        "jacket_helix_turns": {"value": 5.0, "unit": ""},
    }
...
def _validate_artifact_schema(artifact: Mapping[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "winding_temperatures_open": (
            isinstance(artifact.get("winding_temperatures"), Mapping)
            and artifact["winding_temperatures"].get("status") == "OPEN"
        ),
        "cht_open": (
        ...
        "cad_family_bound": artifact.get("cad_family") == "motor_water_jacket_helical",
    }
...
def run_selftest(*, dry_run: bool = False) -> int:
    ...
    case_dir = SCRIPT_DIR / ".openfoam-fia-water-jacket-selftest"
    try:
        ...
        solver_checks = {
            ...
            "inlet_velocity_in_band": 0.5 < geometry.inlet_velocity_m_s < 30.0,
...
def run_live_case(twin_dir: Path, output_path: Path | None = None) -> int:
    """Run and persist one water-jacket duct screen against a live twin."""

    state_path = twin_dir / "state.json"
    inputs, state_hash = load_twin_inputs(state_path)
    geometry = derive_channel_geometry(inputs)
    if not (0.2 < geometry.inlet_velocity_m_s < 40.0):
        raise FiaWaterJacketOpenFoamError(
    ...
    case_dir = twin_dir / "_motor_stack" / "openfoam_fia_water_jacket_case_work"
    write_openfoam_case(case_dir, geometry)
    ...
    try:
        ...
        if solver.u_field_present and solver.converged and (
            ...
            status = "PARTIAL"
    except FiaWaterJacketOpenFoamError as exc:
        pressure = compute_pressure_drop(
            geometry, solver_delta_p_kinematic=None
        )
        try:
        ...
        destination = (
            ...
            else twin_dir / "_motor_stack" / "openfoam_fia_water_jacket_case.json"
        )
    ...
    _atomic_write_json(destination, artifact)
    print(
        "FIA front-kit water-jacket duct screen: "
        f"U≈{geometry.inlet_velocity_m_s:.2f} m/s "
        f"(Re≈{geometry.reynolds_number:,.0f}) at "
        f"{inputs.coolant_flow_l_min:g} L/min / {inputs.coolant_inlet_c:g} °C; "
        f"solver Δp/turn≈{(pressure.solver_delta_p_per_turn_pa or 0):,.1f} Pa; "
        ...
        f"{inputs.helix_turns:g} helix turns on "
        f"OD {inputs.housing_outer_diameter_mm:g} / L {inputs.axial_length_mm:g} mm. "
        "Winding temperatures / CHT remain OPEN; ship_ok is false."
    )
    print(f"Artefact: {destination}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Solve the FIA-bound Formula E front-kit OpenFOAM water-jacket "
            "duct screening case."
        )
    )
