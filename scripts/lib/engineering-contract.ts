...
registerArchetype('co2_mineralisation', (brief: any) => {
  ...
  // 2026-07-06 fix (two-truths regression from the load-reconcile commit above): this
  // used to publish the AGGREGATE of both centralised duct heaters (2 × 75 = 150 kW),
  // but the emitter (co2-mineralisation.ts hot_air_heater_word) prices + rates them as
  // ×2 individual units at dryerHeatDutyKw ("75 kW each") — every OTHER *_kw quantity
  // in this file is a single-equipment rating (gypsum_reactor_agitator_kw,
  // crystalliser_heater_duty_kw, ...), and the deterministic BoM-vs-contract rating
  // check (scripts/deterministic_checks_lib.py::_checks_rating_equals_quantity) reads
  // ONE unit's rating out of the BoM requirement text ("75 kW each") and expects the
  // matching *_kw contract quantity to be that SAME per-unit figure, not a fleet total
  // (BoM H-102 emitted 75 kW vs the old contract 150 kW). Publish the PER-UNIT rating
  // here (== dryer_heat_duty_kw, matching the emitted part) and fold the ×2 multiplier
  // into connectedLoadKw below instead, so the plant-wide total stays the true 150 kW.
  const hotAirProcessHeaterKw = dryerHeatDutyKw          // ONE centralised duct heater's own rating (×2 fitted)
  const hotAirProcessHeaterTotalKw = 2 * hotAirProcessHeaterKw  // both centralised duct heaters (both dryers)
  ...
  const quantities: Record<string, Quantity> = {
    ...
    caco3_vacuum_belt_filter_m3_per_hour: q(caco3SlurryM3PerHour, 'm³/h', 'flow_rate', 'rated', 'module', 'calculator', { source_detail: `caco3_vacuum_belt_filter_m3_per_hour = caco3_slurry_transfer_pump_m3_per_hour = ${totalCaco3TonsPerDay.toFixed(2)} t/d × 1000/24 ÷ ${SLURRY_DENSITY_KG_M3} kg/m³ slurry density = ${caco3SlurryM3PerHour.toFixed(3)} m³/h (the same CaCO3 carbonation-product slurry stream fed to the vacuum belt filter for solid/liquid separation, identical to the transfer-pump flow)`, from: ['caco3_output_t_per_day'] }),
    ...
    mea_distillation_column_m3_per_hour: q(meaCirculationM3PerHour, 'm³/h', 'flow_rate', 'continuous', 'module', 'calculator', { source_detail: `mea_distillation_column_m3_per_hour = mea_circulation_m3_per_hour = ${meaCirculationM3PerHour.toFixed(2)} m³/h (the MEA regenerator, "MEA distillation column", is fed by the SAME rich-amine circulation loop — no separate flow, so the quantity is carried across unchanged)`, from: ['mea_circulation_m3_per_hour'] }),
...
registerArchetype('aquaculture_ras', (brief: any) => {
  ...
  // The recirc loop is LIFE-CRITICAL (brief: stock dies within ~20 min of a flow failure; "duty
  // and standby recirculation pumps"). The N trains carry the flow as DUTY; the pumps get +1
  // INSTALLED STANDBY (N+1) so a single pump failure STILL meets the full flow — a coherent
  // duty/standby scheme (the prior count=N labelled "duty/standby" had ZERO standby: both ran to
  // make the flow). Degassers/drum-filters stay at N (the brief mandates standby on the PUMPS).
  // Universal pattern: a life-critical parallel-pump duty gets an installed +1 standby.
  const recircPumpInstalledCount = recircTrainCount + 1  // N duty + 1 standby
  ...
  const quantities: Record<string, Quantity> = {
    ...
    recirc_pump_count: q(recircPumpInstalledCount, '', 'dimensionless', 'rated', 'system', 'calculator', { source_detail: `${recircTrainCount} DUTY recirculation pumps (= ceil(recirculation_flow ÷ ${SINGLE_UNIT_FLOW_LIMIT_M3H} m³/h single-unit limit), each on the ${perTrainFlowM3H} m³/h per-train flow) + 1 INSTALLED STANDBY = ${recircPumpInstalledCount} pumps total (N+1). A single pump failure still meets the full ${recirculationFlowM3H} m³/h — the brief's mandated duty/standby on the life-critical recirc loop` }),
    // EXPLICIT per-pump duty flow so the synthesiser rates EACH pump for the DUTY-train flow
    // (loop ÷ N duty trains), NOT loop ÷ the N+1 installed count. Without this the universal
    // sizer's perUnitFlow = loop ÷ count divides by the standby too → each pump undersized to
    // loop/(N+1) and the standby vanishes. A throughput key that is NOT a plant-total is kept
    // verbatim as the per-unit rating (universal-contract-sizing.ts buildGroups). Universal:
    // any duty+standby parallel-pump set carries the explicit DUTY per-unit flow.
    recirc_pump_throughput_m3_h: q(perTrainFlowM3H, 'm³/h', 'flow_rate', 'rated', 'module', 'calculator', { source_detail: `PER-PUMP duty flow = ${recirculationFlowM3H} m³/h loop ÷ ${recircTrainCount} DUTY trains = ${perTrainFlowM3H} m³/h (NOT ÷ the ${recircPumpInstalledCount} installed count — the +1 is a STANDBY, not flow-sharing). Each pump is rated for the full per-train duty so a single pump failure is covered by the standby at the same ${perTrainFlowM3H} m³/h` }),
