/**
 * scripts/lib/orchestrator/tools/helium-circulation.ts
 *
 * TypeScript wrapper for `helium_circulation.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'helium_circulation.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const heliumCirculationTool: Tool<any, any> = {
  id: 'helium-circulation:dilution',
  name: 'Helium Circulation Dilution System',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:helium-circulation:dilution',
...
registerTool(heliumCirculationTool)
