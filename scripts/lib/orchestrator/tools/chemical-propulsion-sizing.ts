/**
 * scripts/lib/orchestrator/tools/chemical-propulsion-sizing.ts
 *
 * TypeScript wrapper for `chemical_propulsion_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'chemical_propulsion_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const chemicalPropulsionSizingTool: Tool<any, any> = {
  id: 'chemical-propulsion:sizing',
  name: 'Chemical Propulsion Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['propulsion_thruster_product', 'satellite_smallsat', 'satellite_geo_comsat', 'satellite_interplanetary'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:chemical-propulsion:sizing',
...
registerTool(chemicalPropulsionSizingTool)
