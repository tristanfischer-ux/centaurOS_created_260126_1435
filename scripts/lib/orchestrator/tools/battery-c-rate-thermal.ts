/**
 * scripts/lib/orchestrator/tools/battery-c-rate-thermal.ts
 *
 * TypeScript wrapper for `battery_c_rate_thermal.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'battery_c_rate_thermal.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const batteryCRateThermalTool: Tool<any, any> = {
  id: 'battery-c-rate-thermal:check',
  name: 'Battery C-Rate Thermal Check',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:battery-c-rate-thermal:check',
          tool_id: 'battery-c-rate-thermal:check',
          tool_version: '1.0.0',
        },
        warnings: [],
...
registerTool(batteryCRateThermalTool)
