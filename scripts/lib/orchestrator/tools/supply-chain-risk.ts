/**
 * scripts/lib/orchestrator/tools/supply-chain-risk.ts
 *
 * TypeScript wrapper for `supply_chain_risk.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/scrm
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'supply_chain_risk.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const supplyChainRiskTool: Tool<any, any> = {
  id: 'supply-chain-risk:scoring',
  name: 'Supply Chain Risk',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/scrm',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:supply-chain-risk:scoring',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/scrm',
...
registerTool(supplyChainRiskTool)
