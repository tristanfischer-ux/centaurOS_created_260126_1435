#!/usr/bin/env node
/**
 * Backfill embeddings for investor_portfolio_companies table.
 * Usage: OPENAI_API_KEY=sk-... SUPABASE_SERVICE_ROLE_KEY=... node scripts/backfill-portfolio-embeddings.mjs
 */
...
async function main() {
  ...
  while (true) {
    round++
    const { data, error } = await supabase.from('investor_portfolio_companies')
      .select('id, company_name, sector, stage, description')
    ...
    console.log(`Round ${round}: ${data.length} portfolio companies`)
    for (let i = 0; i < data.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.allSettled(batch.map(async (c) => {
        ...
        const { error: e } = await supabase.from('investor_portfolio_companies').update({ embedding: JSON.stringify(emb) }).eq('id', c.id)
