...
describe("release handoff consistency", () => {
  ...
  it("keeps migration closure instructions and verification gates explicit", () => {
    ...
    expect(handoffDoc).toContain("default branch")
    ...
    expect(handoffDoc).toContain("fill execution metadata + SQL verification output sections")
    expect(handoffDoc).toContain("profiles-rls-verification.sql")
  })
})
