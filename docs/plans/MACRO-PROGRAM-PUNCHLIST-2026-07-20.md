...
| S7 | **Single multi-axis ship card** (tab_floor/self_audit/oem/pcb_readiness/vision) — ships only if ALL pass; never announce from tab_scorecard alone. **DIAGNOSIS (SIGHT 2150):** `compute_verdict` ALREADY folds every axis into `ships` (tab floor incl. PCB via `_sc_pcb`→readiness cap + Renders; self-audit S1; oem S4; process-plant S1). The GAP is DISPLAY: the Exec Summary card shows a single "SHIPS" word (build-excel `verdict_text` L2102 + Exec card), not the per-axis breakdown — a reader can't see WHICH axes passed. S7 = render a multi-axis card (one row per axis: tab-floor / self-audit / oem-vs-ceiling / pcb-readiness / vision-blockout, each PASS/FAIL from its own state field) so SHIPS is provably backed by every axis, never a collapsed word. Vision axis ties to S12 (vision-critic-before-Excel timing). NOT a floor-logic change — a card-render change. | ⬜ NEXT | |
| S8 | Engineering Analysis empty table → not 10 (council M6). SIGHT: 2150 stress table had ZERO rows but scored 10/10 — the ISO 2768 tolerance section auto-PASSes every fab row (boilerplate). FIX (`_eval_engineering_analysis_contract`): tolerance-only → device=verified-OOS (scored:False, off floor); non-device=score_cap 4 (real gap). SIGHT-verified 2150 10→OOS. proveCatch both branches non-vacuous. | ✅ | dfd17129f |
...
| F1e | interconnect: DN25 pipe → micro-tubing for device-scale fluid loops. SOURCE = `connection_sizing.py` `set_device_scale_interconnect()` flag (set from isInstrumentDevice in build_universal_scene.py) → no-flow + material_compat edges default to 6 mm OD micro-tubing, not DN25; real flow still physics-sized. proveCatch interconnect_device_scale_selftest.py. | ✅ | a7ababd45 |
