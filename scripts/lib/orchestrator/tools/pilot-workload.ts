/**
 * scripts/lib/orchestrator/tools/pilot-workload.ts
 *
 * TypeScript wrapper for `pilot_workload.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'pilot_workload.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const pilotWorkloadTool: Tool<any, any> = {
  id: 'pilot-workload:cooper-harper',
  name: 'Pilot Workload Cooper-Harper',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:pilot-workload:cooper-harper',
...
registerTool(pilotWorkloadTool)
