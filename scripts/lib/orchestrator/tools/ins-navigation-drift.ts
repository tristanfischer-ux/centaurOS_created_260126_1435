/**
 * scripts/lib/orchestrator/tools/ins-navigation-drift.ts
 *
 * TypeScript wrapper for `ins_navigation_drift.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/control
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'ins_navigation_drift.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const insNavigationDriftTool: Tool<any, any> = {
  id: 'ins:navigation-drift',
  name: 'INS Navigation Drift Budget',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/control',
  domain: 'control_systems',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['drone', 'auv', 'satellite_smallsat', 'satellite_cubesat'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:ins:navigation-drift',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/control',
...
registerTool(insNavigationDriftTool)
