/**
 * scripts/lib/orchestrator/tools/downwash-recirculation.ts
 *
 * TypeScript wrapper for `downwash_recirculation.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'downwash_recirculation.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const downwashRecirculationTool: Tool<any, any> = {
  id: 'downwash-recirculation:multirotor',
  name: 'Multirotor Downwash Recirculation',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/aero',
  domain: 'aero',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['evtol', 'drone'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:downwash-recirculation:multirotor',
...
registerTool(downwashRecirculationTool)
