/**
 * scripts/lib/orchestrator/tools/dynamic-walking-zmp.ts
 *
 * TypeScript wrapper for `dynamic_walking_zmp.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'dynamic_walking_zmp.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const dynamicWalkingZmpTool: Tool<any, any> = {
  id: 'humanoid:dynamic-walking-zmp',
  name: 'Humanoid Dynamic Walking ZMP',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:humanoid:dynamic-walking-zmp',
...
registerTool(dynamicWalkingZmpTool)
