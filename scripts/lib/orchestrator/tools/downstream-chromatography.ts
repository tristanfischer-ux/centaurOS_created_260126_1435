/**
 * scripts/lib/orchestrator/tools/downstream-chromatography.ts
 *
 * TypeScript wrapper for `downstream_chromatography.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/bio
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'downstream_chromatography.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const downstreamChromatographyTool: Tool<any, any> = {
  id: 'downstream-chromatography:sizing',
  name: 'Downstream Chromatography Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/bio',
  domain: 'biochemistry',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:downstream-chromatography:sizing',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/bio',
...
registerTool(downstreamChromatographyTool)
