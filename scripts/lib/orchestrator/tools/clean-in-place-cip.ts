/**
 * scripts/lib/orchestrator/tools/clean-in-place-cip.ts
 *
 * TypeScript wrapper for `clean_in_place_cip.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'clean_in_place_cip.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const cleanInPlaceCipTool: Tool<any, any> = {
  id: 'clean-in-place:cip',
  name: 'Clean-in-Place CIP Sizing',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:clean-in-place:cip',
...
registerTool(cleanInPlaceCipTool)
