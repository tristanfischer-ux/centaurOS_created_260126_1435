/**
 * scripts/lib/orchestrator/tools/cable-thermal.ts
 *
 * TypeScript wrapper for `cable_thermal.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'cable_thermal.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const cableThermalTool: Tool<any, any> = {
  id: 'cable:thermal-rating',
  name: 'Cable Thermal Rating IEC 62893',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/ev',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['ev_charger', 'bess'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:cable:thermal-rating',
