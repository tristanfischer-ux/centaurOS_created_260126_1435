/**
 * scripts/lib/orchestrator/tools/pressure-vessel.ts
 *
 * TypeScript wrapper for `pressure_vessel.py` — subprocess to repo .venv.
 *
 * Auto-generated 2026-05-22 by /tmp/wrap-gen/gen.py. The Python wrapper
 * is the canonical implementation; this TS wrapper marshals the JSON
 * input + output and registers the tool with the orchestrator registry.
 *
 * License: free-proprietary. Source: internal://forgeos/structural
 */

import { registerTool } from '../registry'
import type { Tool, ToolResult } from '../types'
import { spawnSync } from 'child_process'
import { resolve } from 'path'

const PYTHON_SCRIPT = resolve(__dirname, 'python', 'pressure_vessel.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const pressureVesselTool: Tool<any, any> = {
  id: 'pressure-vessel:design',
  // Jurisdiction-NEUTRAL name (2026-06-04, gate-19): the previous "ASME BPVC" name
  // hard-cited a US pressure code that propagated into the narrative and failed the
  // jurisdictional-standards audit on UK/EU briefs. The wall calc (hoop stress) is
  // code-agnostic; the design CODE (PED/BS EN 13445 for UK, ASME VIII for US) is
  // cited in the jurisdiction-aware compliance/hazards layer, not the tool name.
  name: 'Pressure Vessel Wall Design (hoop-stress sizing)',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/structural',
  domain: 'mechanical',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['auv', 'bess', 'wind_turbine', 'h2_electrolyser', '3d_printer_fdm', 'cnc_machine', 'satellite_smallsat', 'satellite_cubesat', 'satellite_geo_comsat', 'ventilator', 'co2_mineralisation', 'e_fuel_synthesis'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    const t0 = Date.now()
    const payload = JSON.stringify(input)
    const proc = spawnSync(VENV_PYTHON, [PYTHON_SCRIPT], {
      input: payload,
      encoding: 'utf-8',
      timeout: 60_000,
    })
    const duration_ms = Date.now() - t0
    if (proc.status !== 0) {
      return {
        ok: false,
        output: null,
        provenance: {
          source: 'tool:pressure-vessel:design',
          tool_id: 'pressure-vessel:design',
          tool_version: '1.0.0',
        },
        warnings: [],
        error: `Python exit ${proc.status}: ${proc.stderr?.slice(0, 400) ?? '(no stderr)'}`,
      }
    }
    let output: any
    try {
      output = JSON.parse(proc.stdout)
    } catch (err) {
      return {
        ok: false,
        output: null,
        provenance: {
          source: 'tool:pressure-vessel:design',
          tool_id: 'pressure-vessel:design',
          tool_version: '1.0.0',
        },
        warnings: [],
        error: `JSON parse: ${(err as Error).message}; stdout: ${proc.stdout.slice(0, 200)}`,
      }
    }
    return {
      ok: true,
      output,
      provenance: {
        source: 'tool:pressure-vessel:design',
        tool_id: 'pressure-vessel:design',
        tool_version: '1.0.0',
        tool_license: 'free-proprietary',
        tool_source_url: 'internal://forgeos/structural',
        invocation_input: input,
        invocation_output_field: '(multiple)',
        pinned_versions: { python: '3.14.4' },
        timestamp: new Date().toISOString(),
        duration_ms,
      },
      warnings: [],
    }
  },
}
registerTool(pressureVesselTool)
