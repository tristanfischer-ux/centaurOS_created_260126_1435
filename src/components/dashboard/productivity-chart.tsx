...
/**
 * Productivity Chart - Task completion bar chart for the last 7 days.
 *
 * @description Renders only when there are at least 3 days with activity.
 * For brand-new users this returns null so the dashboard isn't cluttered
 * with empty bars.
 */
export function ProductivityChart({ data }: ProductivityChartProps) {
  // Don't render for new users with insufficient data
  if (!data || data.length === 0) return null

  const daysWithActivity = data.filter(d => d.myTasks + d.teamTasks > 0).length
    ...
    <div className="bg-card rounded-2xl shadow-sm border overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-center gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">You ({totalMyTasks})</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-status-info" />
            <span className="text-xs text-muted-foreground">Team ({totalTeamTasks})</span>
          </div>
        </div>
              ...
              <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
                ...
                <div className="relative w-full h-full flex flex-col justify-end">
                  <div className="relative w-full flex flex-col-reverse gap-0.5">
                    {myPercent > 0 && (
                      <div
                        className="w-full bg-international-orange rounded-t-lg transition-all duration-500 hover:bg-international-orange-hover group relative"
                    ...
                    {teamPercent > 0 && (
                      <div
                        className={cn(
                          'w-full bg-status-info transition-all duration-500 hover:bg-electric-blue group relative',
      ...
      <div className="px-6 pb-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-muted/50 rounded-xl">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-status-info">{totalTeamTasks}</p>
