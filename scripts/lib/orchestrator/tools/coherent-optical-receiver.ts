/**
 * scripts/lib/orchestrator/tools/coherent-optical-receiver.ts
 *
 * TypeScript wrapper for `coherent_optical_receiver.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'coherent_optical_receiver.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const coherentOpticalReceiverTool: Tool<any, any> = {
  id: 'coherent-optical-receiver:dpsk-qpsk',
  name: 'Coherent Optical Receiver',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/optics',
  domain: 'photonics',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['fso', 'ground_station'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:coherent-optical-receiver:dpsk-qpsk',
...
registerTool(coherentOpticalReceiverTool)
