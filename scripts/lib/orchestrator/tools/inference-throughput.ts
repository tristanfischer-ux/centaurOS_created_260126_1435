/**
 * scripts/lib/orchestrator/tools/inference-throughput.ts
 *
 * TypeScript wrapper for `inference_throughput.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/edge
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'inference_throughput.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const inferenceThroughputTool: Tool<any, any> = {
  id: 'inference-throughput:edge',
  name: 'Inference Throughput Edge',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/edge',
  domain: 'control_systems',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['edge_ai'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:inference-throughput:edge',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/edge',
...
registerTool(inferenceThroughputTool)
