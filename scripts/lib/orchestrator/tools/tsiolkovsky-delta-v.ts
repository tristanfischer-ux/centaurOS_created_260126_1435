/**
 * scripts/lib/orchestrator/tools/tsiolkovsky-delta-v.ts
 *
 * TypeScript wrapper for `tsiolkovsky_delta_v.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'tsiolkovsky_delta_v.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const tsiolkovskyDeltaVTool: Tool<any, any> = {
  id: 'tsiolkovsky:delta-v',
  name: 'Tsiolkovsky ΔV Equation',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:tsiolkovsky:delta-v',
...
registerTool(tsiolkovskyDeltaVTool)
