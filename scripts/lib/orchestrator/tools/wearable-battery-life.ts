/**
 * scripts/lib/orchestrator/tools/wearable-battery-life.ts
 *
 * TypeScript wrapper for `wearable_battery_life.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'wearable_battery_life.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const wearableBatteryLifeTool: Tool<any, any> = {
  id: 'wearable-battery:life',
  name: 'Wearable Coin-Cell Battery Life',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/medical',
  domain: 'battery',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['cgm'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:wearable-battery:life',
...
registerTool(wearableBatteryLifeTool)
