...
- [ ] Standalone task product-tagging UI (the Create Task dialog / Task detail panel gets a product `<select>`, mirroring the Edit Objective dialog shipped 2026-04-18). *(Deferred: tasks.product_id exists in DB but is not wired through `src/actions/tasks.ts` — needs server-action update to accept/persist product_id, then UI. Also the "mirrored" Edit Objective dialog doesn't currently surface product_id either; grep shows no product_id references in objectives dialog. Multi-file feature, not a quick polish.)*
- [ ] Objectives detail: reverse link chip `Linked to Product X` (mirror of the `LinkedProductChip` shipped on CAD Lab; reuse the pattern). *(Note: /objectives/[id] and /objectives both redirect to /new-objectives. The live view is new-objectives/objective-detail-panel.tsx, which uses ObjectiveWithTasks type from types.ts — product_id is NOT currently on that type or fetched by new-objectives/page.tsx query. Shipping this properly requires: (1) add `product_id: string | null` to ObjectiveWithTasks in types.ts, (2) add `product_id` to the SELECT in new-objectives/page.tsx, (3) render <LinkedProductChip productId={...} /> in objective-detail-panel. Deferred to avoid multi-file plumbing alongside the simpler Phase 3 polish items.)*
