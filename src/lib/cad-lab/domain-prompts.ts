...

{
  ...
  "cadGeometryPrompt": "300-500 words. A complete prompt for a text-to-CAD AI that generates parametric STEP/B-Rep geometry. Describe the product's overall envelope dimensions, then each major section's shape primitives (extrusions, revolves, sweeps, fillets, chamfers) and their dimensional relationships. Specify assembly hierarchy: which bodies are separate parts vs. features of the same solid. Include interface geometry: how sections mate (bolt patterns, press fits, snap features, clearance gaps). Use absolute dimensions from source data where available, proportional relationships otherwise. Specify wall thicknesses, draft angles, and fillet radii for manufacturable geometry. Do NOT include rendering instructions, colors, camera angles, or visual styling — focus purely on parametric shape definition.",
