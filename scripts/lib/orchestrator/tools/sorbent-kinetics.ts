/**
 * scripts/lib/orchestrator/tools/sorbent-kinetics.ts
 *
 * TypeScript wrapper for `sorbent_kinetics.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'sorbent_kinetics.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const sorbentKineticsTool: Tool<any, any> = {
  id: 'dac:sorbent-kinetics',
  name: 'DAC Sorbent Kinetics',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:dac:sorbent-kinetics',
...
registerTool(sorbentKineticsTool)
