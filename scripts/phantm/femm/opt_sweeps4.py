"""PHANTM optimisation — batch 4: council head-to-head.
...
Run: cd scripts/phantm && python -m femm.opt_sweeps3 → out/opt/opt-sweeps-4.json
"""
...
POINTS = [
    # council batch: d.45 vs d.40 full stack, joint Pm — worst-registration objective
    ("d45 stack N52 Pm0.40", 0.020, 0.45, 1.50, 2.0, 1.45, 0.40),
    ("d45 stack N52 Pm0.50", 0.020, 0.45, 1.50, 2.0, 1.45, 0.50),
    ("d40 stack N52 Pm0.50", 0.020, 0.40, 1.50, 2.0, 1.45, 0.50),
]
...
def main():
    ...
    json.dump({"rows": rows, "runtime_s": round(time.time() - t0, 1)},
              open(os.path.join(OUT, "opt-sweeps-4.json"), "w"), indent=1)
    print(f"\nwrote out/opt/opt-sweeps-4.json ({time.time()-t0:.0f} s)")
