/**
 * scripts/lib/orchestrator/tools/qubit-chip-thermal.ts
 *
 * TypeScript wrapper for `qubit_chip_thermal.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'qubit_chip_thermal.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const qubitChipThermalTool: Tool<any, any> = {
  id: 'qubit-chip-thermal:base-plate',
  name: 'Qubit Chip Thermal Base Plate',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/quantum',
  domain: 'control_systems',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['quantum_computer', 'cryostat'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:qubit-chip-thermal:base-plate',
...
registerTool(qubitChipThermalTool)
