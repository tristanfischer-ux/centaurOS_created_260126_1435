/**
 * scripts/lib/orchestrator/tools/thermal-envelope.ts
 *
 * TypeScript wrapper for `thermal_envelope.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'thermal_envelope.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const thermalEnvelopeTool: Tool<any, any> = {
  id: 'thermal-envelope:ladder',
  name: 'Thermal Resistance Ladder Envelope',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/thermal',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['edge_ai', 'bess', 'ev_charger', 'cnc_machine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:thermal-envelope:ladder',
...
registerTool(thermalEnvelopeTool)
