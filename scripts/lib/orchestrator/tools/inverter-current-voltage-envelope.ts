/**
 * scripts/lib/orchestrator/tools/inverter-current-voltage-envelope.ts
 *
 * TypeScript wrapper for `inverter_current_voltage_envelope.py` — subprocess to repo .venv.
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'inverter_current_voltage_envelope.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const InverterCurrentVoltageEnvelopeTool: Tool<any, any> = {
  id: 'inverter:current-voltage-envelope',
  name: 'Inverter DC/AC Current-Voltage Envelope',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    const provenance: Provenance = {
      source: 'tool:inverter:current-voltage-envelope',
...
registerTool(InverterCurrentVoltageEnvelopeTool)
