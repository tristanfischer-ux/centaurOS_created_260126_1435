/**
 * scripts/lib/orchestrator/tools/network-bandwidth.ts
 *
 * TypeScript wrapper for `network_bandwidth.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'network_bandwidth.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const networkBandwidthTool: Tool<any, any> = {
  id: 'network-bandwidth:budget',
  name: 'Network Bandwidth Budget',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:network-bandwidth:budget',
...
registerTool(networkBandwidthTool)
