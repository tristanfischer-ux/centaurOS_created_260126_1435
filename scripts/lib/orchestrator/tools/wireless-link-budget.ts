/**
 * scripts/lib/orchestrator/tools/wireless-link-budget.ts
 *
 * TypeScript wrapper for `wireless_link_budget.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/rf
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'wireless_link_budget.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const wirelessLinkBudgetTool: Tool<any, any> = {
  id: 'wireless-link-budget:friis',
  name: 'Wireless Link Budget (Friis)',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/rf',
  domain: 'power_electronics',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['cgm', 'satellite_smallsat', 'satellite_cubesat', 'ground_station'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:wireless-link-budget:friis',
...
registerTool(wirelessLinkBudgetTool)
