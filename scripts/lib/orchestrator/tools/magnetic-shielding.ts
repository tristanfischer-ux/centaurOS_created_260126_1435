/**
 * scripts/lib/orchestrator/tools/magnetic-shielding.ts
 *
 * TypeScript wrapper for `magnetic_shielding.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'magnetic_shielding.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const magneticShieldingTool: Tool<any, any> = {
  id: 'magnetic-shielding:mu-metal',
  name: 'Magnetic Shielding Mu-metal',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:magnetic-shielding:mu-metal',
...
registerTool(magneticShieldingTool)
