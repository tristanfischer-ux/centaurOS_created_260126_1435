/**
 * scripts/lib/orchestrator/tools/irrigation-pump-sizing.ts
 *
 * TypeScript wrapper for `irrigation_pump_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'irrigation_pump_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const irrigationPumpSizingTool: Tool<any, any> = {
  id: 'irrigation:pump-sizing',
  name: 'Irrigation Pump Sizing',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:irrigation:pump-sizing',
...
registerTool(irrigationPumpSizingTool)
