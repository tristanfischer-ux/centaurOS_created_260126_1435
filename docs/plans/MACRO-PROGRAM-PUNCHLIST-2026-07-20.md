...
| F1f | **Design identity / scale lock** (Cursor T8). **Layer 0 DONE (e4656ca0d):** `design-scale-tier.ts` `deriveDesignScaleTier`+`buildDesignIdentity` → immutable `state.designIdentity.scale_tier` (handheld/benchtop/cabinet/plant/field/unknown) from envelope+power+working-volume PHYSICS, wired at chain state-save; SIGHT-verified 2150→benchtop despite "heater"; proveCatch design-scale-tier-selftest.ts. **Layers 1–4 OPEN (next increment):** L1 hard scale-veto in relevance-sweep.ts/bootstrap-tool-plan.ts (add `scale_tiers[]` to Tool interface + tag plant-only tools + veto before LLM); L2 scale-gated RAG/class-graph; L3 homonym-safe word-expand; L4 gate-34 PLANT_SCALE markers. |
