/**
 * scripts/lib/orchestrator/tools/battery-eclipse-cycle.ts
 *
 * TypeScript wrapper for `battery_eclipse_cycle.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'battery_eclipse_cycle.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const batteryEclipseCycleTool: Tool<any, any> = {
  id: 'battery-eclipse:cycle',
  name: 'Battery Eclipse Cycle',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'battery',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['satellite_smallsat', 'satellite_cubesat', 'satellite_geo_comsat'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:battery-eclipse:cycle',
...
registerTool(batteryEclipseCycleTool)
