/**
 * scripts/lib/orchestrator/tools/osmosis-membrane.ts
 *
 * TypeScript wrapper for `osmosis_membrane.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'osmosis_membrane.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const osmosisMembraneTool: Tool<any, any> = {
  id: 'osmosis-membrane:dialyser',
  name: 'Osmosis Membrane Dialyser Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/medical',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['dialysis_machine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:osmosis-membrane:dialyser',
...
registerTool(osmosisMembraneTool)
