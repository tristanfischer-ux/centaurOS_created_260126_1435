...
> **TO MAKE DEFAULT (remaining B):** (a) per-archetype VISUAL sign-off — det render vs the optimiser render on water/BESS/RAS/CO2 (det looked slightly tight/small → maybe region-bay-width tuning); (b) replication (qty-N instances) + envelope-shell coherence under det-coords (those still use old paths — verify they enclose the det-placed parts); (c) surface dl's cabinet nodes in the draw step; (d) flip default → full chain → re-attack. Determinism no longer blocks; only layout-quality sign-off does.
>
