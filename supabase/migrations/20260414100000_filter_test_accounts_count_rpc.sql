-- ═══════════════════════════════════════════════════════════════════════
-- Migration: Add test/demo account filters to get_directory_expert_count
--
-- Purpose: The get_directory_expert_count RPC was not updated when
-- test account filtering was added to get_directory_experts (migration
-- 20260411100000). This caused the pagination count to include test
-- accounts even though the listing excluded them.
--
-- Also adds 'demo' pattern filtering to all three directory RPCs.
-- ═══════════════════════════════════════════════════════════════════════


-- ═══ Fix get_directory_expert_count ═══════════════════════════════════
...
BEGIN
    ...
    WHERE pp.is_public = TRUE
      ...
      AND p.full_name NOT ILIKE '%qa %'
      AND p.full_name NOT ILIKE '%demo%'
      AND (
          p_role IS NULL
...
-- ═══ Add 'demo' filter to get_directory_experts ═══════════════════════
...
-- ═══ Add 'demo' filter to get_directory_expert_by_slug ════════════════
