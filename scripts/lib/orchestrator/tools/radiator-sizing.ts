/**
 * scripts/lib/orchestrator/tools/radiator-sizing.ts
 *
 * TypeScript wrapper for `radiator_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'radiator_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const radiatorSizingTool: Tool<any, any> = {
  id: 'radiator:spacecraft-sizing',
  name: 'Spacecraft Radiator Sizing',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/spacecraft',
  domain: 'thermal',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['satellite_smallsat', 'satellite_cubesat', 'satellite_geo_comsat', 'satellite_interplanetary', 'haps', 'edge_ai'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:radiator:spacecraft-sizing',
...
registerTool(radiatorSizingTool)
