...
interface PdfInput {
    ...
    /** Loop 8 (CONVERGED-DOC-STRUCTURE.md Tier 1) — 1-page condensed
     *  view of the project. Renders as section 0/page 2 of every PDF
     *  so investors / CMs / certification reviewers read it first. Null
     *  when the executive-summary stage hasn't run yet. */
...
// summary itself is generated by run-executive-summary.ts as plain
// paragraphs separated by blank lines; we split on \n\n and render
// each block. No headers, no markdown, no fancy layout — by design.
    ...
    return (
        <Page size="A4" style={styles.page} wrap>
            ...
            <Text style={[styles.small, { marginBottom: 12 }]}>
                One-page synthesis for investors, contract manufacturers, and
                certification reviewers. Numbers cited here are sourced from
                the same engine state that produces the rest of this pack.
            </Text>
            {paragraphs.map((p, i) => (
                <Text key={i} style={[styles.para, { marginBottom: 8 }]}>
                    {p}
                </Text>
            ))}
            <PdfFooter label="Executive summary" />
        </Page>
    )
}
...
function ForgeProjectPdf({ data }: { data: PdfInput }): React.ReactElement {
    ...
    return (
        <Document>
            ...
            {/* Executive summary (Loop 8 — Tier 1). Renders as page 2
             *  before the TOC so investors / CMs / certification reviewers
             *  see the one-page synthesis first. Skipped silently when
             *  the executive-summary stage hasn't run on this project. */}
...
async function exportProjectPdfInternal(
...
): Promise<ExportProjectPdfResult> {
    {
        ...
        const { data: project, error: projectErr } = await admin
            ...
            .select(
                "id, foundry_id, name, subject, modules, research, ai_cost_estimates, reviews, diagnostic_answers, design_revision, created_at, brief_locked_at, shipped_at, system_illustration_url, interior_overview_url, concept_render_url, dimension_sheet, spatial_plan, proofread_findings, feasibility_verdict, executive_summary",
            )
            .eq("id", projectId)
            .maybeSingle()
        ...
        const pdfInput: PdfInput = {
            ...
            foundryName,
            executiveSummary:
                typeof (project as Record<string, unknown>).executive_summary === "string" &&
                ((project as Record<string, unknown>).executive_summary as string).trim().length > 0
                    ? ((project as Record<string, unknown>).executive_summary as string)
                    : null,
