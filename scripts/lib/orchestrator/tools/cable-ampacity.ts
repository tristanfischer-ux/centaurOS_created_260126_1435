/**
 * scripts/lib/orchestrator/tools/cable-ampacity.ts
 *
 * TypeScript wrapper for `cable_ampacity.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'cable_ampacity.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const cableAmpacityTool: Tool<any, any> = {
  id: 'cable:ampacity',
  name: 'Cable Ampacity NEC/IEC',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/electrical',
  domain: 'power_electronics',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['bess', 'ev_charger', 'h2_electrolyser', 'wind_turbine', 'solar_inverter'].includes(envelope.class) },
...
registerTool(cableAmpacityTool)
