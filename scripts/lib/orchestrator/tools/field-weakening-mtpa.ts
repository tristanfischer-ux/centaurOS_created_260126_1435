/**
 * scripts/lib/orchestrator/tools/field-weakening-mtpa.ts
 *
 * TypeScript wrapper for `field_weakening_mtpa.py` — subprocess to repo .venv.
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'field_weakening_mtpa.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const FieldWeakeningMtpaTool: Tool<any, any> = {
  id: 'inverter:field-weakening-mtpa',
  name: 'IPMSM Field-Weakening / MTPA Current Setpoints',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    const provenance: Provenance = {
      source: 'tool:inverter:field-weakening-mtpa',
...
registerTool(FieldWeakeningMtpaTool)
