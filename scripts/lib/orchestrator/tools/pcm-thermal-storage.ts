/**
 * scripts/lib/orchestrator/tools/pcm-thermal-storage.ts
 *
 * TypeScript wrapper for `pcm_thermal_storage.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'pcm_thermal_storage.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const pcmThermalStorageTool: Tool<any, any> = {
  id: 'pcm:thermal-storage',
  ...
  applicable_to(envelope) { return ['satellite_smallsat', 'haps', 'bess'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:pcm:thermal-storage',
...
registerTool(pcmThermalStorageTool)
