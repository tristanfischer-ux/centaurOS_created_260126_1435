/**
 * scripts/lib/orchestrator/tools/monod-kinetics.ts
 *
 * TypeScript wrapper for `monod_kinetics.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'monod_kinetics.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const monodKineticsTool: Tool<any, any> = {
  id: 'monod:growth-kinetics',
  name: 'Monod Growth Kinetics',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:monod:growth-kinetics',
...
registerTool(monodKineticsTool)
