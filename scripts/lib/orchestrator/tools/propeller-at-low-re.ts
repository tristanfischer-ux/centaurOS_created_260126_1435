/**
 * scripts/lib/orchestrator/tools/propeller-at-low-re.ts
 *
 * TypeScript wrapper for `propeller_at_low_re.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'propeller_at_low_re.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const propellerAtLowReTool: Tool<any, any> = {
  id: 'propeller:low-re-bemt',
  name: 'Propeller BEMT at Low Reynolds',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:propeller:low-re-bemt',
...
registerTool(propellerAtLowReTool)
