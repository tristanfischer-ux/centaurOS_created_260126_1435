/**
 * scripts/lib/orchestrator/tools/mppt-tracking-model.ts
 *
 * TypeScript wrapper for `mppt_tracking_model.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/solar
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'mppt_tracking_model.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const mpptTrackingModelTool: Tool<any, any> = {
  id: 'mppt:sandia-tracking',
  name: 'MPPT Sandia Tracking Model',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/solar',
  domain: 'power_electronics',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['solar_inverter', 'haps', 'satellite_smallsat'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:mppt:sandia-tracking',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/solar',
...
registerTool(mpptTrackingModelTool)
