/**
 * scripts/lib/orchestrator/tools/electrolyser-efficiency.ts
 *
 * TypeScript wrapper for `electrolyser_efficiency.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/h2
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'electrolyser_efficiency.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const electrolyserEfficiencyTool: Tool<any, any> = {
  id: 'electrolyser:efficiency',
  name: 'PEM Electrolyser Efficiency',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/h2',
  domain: 'process',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['h2_electrolyser'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:electrolyser:efficiency',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/h2',
...
registerTool(electrolyserEfficiencyTool)
