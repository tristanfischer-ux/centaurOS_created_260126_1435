/**
 * scripts/lib/orchestrator/tools/dexterity-kinematics.ts
 *
 * TypeScript wrapper for `dexterity_kinematics.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'dexterity_kinematics.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const dexterityKinematicsTool: Tool<any, any> = {
  id: 'humanoid:dexterity-kinematics',
  name: 'Humanoid Dexterity Kinematics',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:humanoid:dexterity-kinematics',
...
registerTool(dexterityKinematicsTool)
