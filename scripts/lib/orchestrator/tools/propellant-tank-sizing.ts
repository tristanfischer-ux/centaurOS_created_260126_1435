/**
 * scripts/lib/orchestrator/tools/propellant-tank-sizing.ts
 *
 * TypeScript wrapper for `propellant_tank_sizing.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'propellant_tank_sizing.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const propellantTankSizingTool: Tool<any, any> = {
  id: 'propellant-tank:sizing',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:propellant-tank:sizing',
...
registerTool(propellantTankSizingTool)
