/**
 * scripts/lib/orchestrator/tools/duty-cycle-energy.ts
 *
 * TypeScript wrapper for `duty_cycle_energy.py` — subprocess to repo .venv.
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'duty_cycle_energy.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const DutyCycleEnergyTool: Tool<any, any> = {
  id: 'powertrain:duty-cycle-energy',
  name: 'Race / Duty-Cycle Net Electrical Energy',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    const provenance: Provenance = {
      source: 'tool:powertrain:duty-cycle-energy',
...
registerTool(DutyCycleEnergyTool)
