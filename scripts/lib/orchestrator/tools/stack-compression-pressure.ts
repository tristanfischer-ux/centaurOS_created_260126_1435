/**
 * scripts/lib/orchestrator/tools/stack-compression-pressure.ts
 *
 * TypeScript wrapper for `stack_compression_pressure.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'stack_compression_pressure.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const stackCompressionPressureTool: Tool<any, any> = {
  id: 'stack-compression:pressure',
  name: 'Stack Compression Pressure',
  version: '1.0.0',
  license: 'proprietary',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:stack-compression:pressure',
...
registerTool(stackCompressionPressureTool)
