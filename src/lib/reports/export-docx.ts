...
async function buildCover(data: CoverSectionData, ctx: ExportContext): Promise<DocChild[]> {
  ...
  const coverImageUrl = (data as any).coverImageUrl as string | undefined
  if (coverImageUrl) {
...
async function buildKeyMetrics(data: KeyMetricsSectionData, ctx: ExportContext): Promise<DocChild[]> {
  const num = ctx.templateId === 'board-pack' ? ctx.sectionNumber : undefined
  const paragraphs: DocChild[] = [sectionHeading('Key Metrics', num !== undefined ? { number: num } : undefined)]

  const chartImageUrl = (data as any).chartImageUrl as string | undefined
  if (chartImageUrl) {
    const imgBuffer = await fetchImageAsBuffer(chartImageUrl)
    if (imgBuffer) {
...
async function buildCompletionTrend(data: CompletionTrendSectionData, ctx: ExportContext): Promise<DocChild[]> {
  ...
  // When a chartImageUrl is present, embed it above the table for visual reference.
  const num = ctx.templateId === 'board-pack' ? ctx.sectionNumber : undefined
...
type SectionBuilder = (data: Record<string, unknown>, ctx: ExportContext) => Promise<DocChild[]> | DocChild[]
...
export async function exportReportAsDOCX(document: ReportDocument): Promise<Blob> {
  ...
  for (const section of document.sections) {
    ...
    if (builder) {
      ...
      const content = await builder(section.data as unknown as Record<string, unknown>, ctx)
