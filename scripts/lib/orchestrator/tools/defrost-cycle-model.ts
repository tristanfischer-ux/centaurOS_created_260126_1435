/**
 * scripts/lib/orchestrator/tools/defrost-cycle-model.ts
 *
 * TypeScript wrapper for `defrost_cycle_model.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'defrost_cycle_model.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const defrostCycleModelTool: Tool<any, any> = {
  id: 'defrost-cycle:model',
  name: 'Defrost Cycle Energy Model',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:defrost-cycle:model',
...
registerTool(defrostCycleModelTool)
