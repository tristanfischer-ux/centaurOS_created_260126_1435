...
export function BrainstormingCouncilView({ userId }: BrainstormingCouncilViewProps) {
    ...
    const generatedCoverUrlRef = useRef<string | null>(null)
    ...
    const handleConvene = useCallback((e: React.FormEvent | React.MouseEvent) => {
        ...
        ;(async () => {
            ...
            // Reset asset generation buttons for the new session but keep
            // URLs intact — completeMeetingThread's after() block generates
            // cover/audio asynchronously, and clearing URLs here caused images
            // to flash then vanish.
            ...
            try {
                if (liveThreadId) {
                    ...
                    if (completeResult.ok) {
                        ...
                        // Cover image + audio polling removed 2026-05-19 — both
                        // features are paused. No assets to wait on.
                    }
                } else {
    ...
    function handleReset() {
        ...
        generatedCoverUrlRef.current = null
    ...
    return (
        <div className="bc-page">
                    ...
                    {session.specialistResponses.map((arrived) => {
                        ...
                        return (
                            <div key={arrived.id} className="bc-specialist-card" style={isError ? { opacity: 0.6 } : undefined}>
                                ...
                                {!isError && arrived.fallbackModelUsed && (
                                    <div className={`bc-sig-close ${sigColorClass}`} style={{ marginTop: "auto" }}>
                                        <span
            ...
            {/* PAUSED 2026-05-19 — graphic-summary + audio-transcript panel and
                the inline cover/audio render were removed. The original blocks
                live in git history (commit prior to this change) and the
                underlying server actions (brainstorm-cover.ts, brainstorm-audio.ts)
                stay intact for future reinstatement. */}
