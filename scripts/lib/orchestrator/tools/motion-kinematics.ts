/**
 * scripts/lib/orchestrator/tools/motion-kinematics.ts
 *
 * TypeScript wrapper for `motion_kinematics.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/motion
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'motion_kinematics.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const motionKinematicsTool: Tool<any, any> = {
  id: 'motion-kinematics:steps-per-mm',
  name: 'Motion Steps-per-mm Kinematics',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/motion',
  domain: 'control_systems',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['3d_printer_fdm', 'cnc_machine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:motion-kinematics:steps-per-mm',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/motion',
...
registerTool(motionKinematicsTool)
