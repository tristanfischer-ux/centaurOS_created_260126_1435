/**
 * scripts/lib/orchestrator/tools/biocompatibility-check.ts
 *
 * TypeScript wrapper for `biocompatibility_check.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'biocompatibility_check.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const biocompatibilityCheckTool: Tool<any, any> = {
  id: 'biocompatibility:iso-10993',
  name: 'Biocompatibility ISO 10993 Lookup',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/medical',
  domain: 'standards',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['cgm', 'ventilator', 'dialysis_machine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:biocompatibility:iso-10993',
...
registerTool(biocompatibilityCheckTool)
