/**
 * scripts/lib/orchestrator/tools/noise-emission-dba.ts
 *
 * TypeScript wrapper for `noise_emission_dba.py` — subprocess to repo .venv.
 *
 ...
 *
 * License: free-proprietary. Source: internal://forgeos/acoustics
 */
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'noise_emission_dba.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const noiseEmissionDbaTool: Tool<any, any> = {
  id: 'noise-emission:dba',
  name: 'Noise Emission dBA Outdoor Unit',
  version: '1.0.0',
  license: 'free-proprietary',
  source_url: 'internal://forgeos/acoustics',
  domain: 'mechanical',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['heat_pump_residential', 'drone', 'e_bike', 'wind_turbine'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:noise-emission:dba',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/acoustics',
...
registerTool(noiseEmissionDbaTool)
