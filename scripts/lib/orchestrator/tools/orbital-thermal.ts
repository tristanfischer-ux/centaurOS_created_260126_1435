/**
 * scripts/lib/orchestrator/tools/orbital-thermal.ts
 *
 * TypeScript wrapper for `orbital_thermal.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'orbital_thermal.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const orbitalThermalTool: Tool<any, any> = {
  id: 'orbital-thermal:sat-balance',
  name: 'Orbital Thermal Balance',
  version: '1.0.0',
  ...
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:orbital-thermal:sat-balance',
...
registerTool(orbitalThermalTool)
