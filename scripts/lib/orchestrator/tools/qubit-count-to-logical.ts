/**
 * scripts/lib/orchestrator/tools/qubit-count-to-logical.ts
 *
 * TypeScript wrapper for `qubit_count_to_logical.py` — subprocess to repo .venv.
 *
...
const PYTHON_SCRIPT = resolve(__dirname, 'python', 'qubit_count_to_logical.py')
const VENV_PYTHON = resolve(__dirname, '..', '..', '..', '..', '.venv', 'bin', 'python3')

export const qubitCountToLogicalTool: Tool<any, any> = {
  id: 'qubit-count-to-logical:error-correction',
  name: 'Qubit Count To Logical Error Correction',
  version: '1.0.0',
  license: 'proprietary',
  source_url: 'internal://forgeos/quantum',
  domain: 'control_systems',
  pinned_environment: { python: '3.14.4' },
  applicable_to(envelope) { return ['quantum_computer'].includes(envelope.class) },
  async invoke(input: any): Promise<ToolResult<any>> {
    ...
    if (proc.status !== 0) {
      return {
        ...
        output: null,
        provenance: {
          source: 'tool:qubit-count-to-logical:error-correction',
    ...
    return {
      ...
      provenance: {
        ...
        tool_source_url: 'internal://forgeos/quantum',
...
registerTool(qubitCountToLogicalTool)
