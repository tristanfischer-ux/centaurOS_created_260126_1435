#!/usr/bin/env bash
#
# Run the FIA-bound water-jacket rectangular-duct OpenFOAM case.
# Uses native OpenFOAM when available, otherwise microfluidica/openfoam:14
# via Docker/Colima (linux/arm64).
#
# Usage:
#   openfoam_fia_water_jacket_case.sh <case_dir>
#
...
fail()
{
  printf 'OPENFOAM_FIA_WATER_JACKET_FAIL: %s\n' "$*" >&2
  exit 1
}
...
run_case()
{
  ...
  printf 'OPENFOAM_FIA_WATER_JACKET_PASS: mesh, solve, U and p fields verified\n'
}
